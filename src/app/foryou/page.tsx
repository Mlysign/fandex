  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="max-w-md mx-auto px-6 py-8">
        ...
        {loading && !current && <div className="text-center py-24 text-neutral-500 animate-pulse">Finding picks for you…</div>}
        ...
        {current && (
          <div className="relative h-[520px] select-none">
            ...
            >
              {current.posterUrl
                ? <img src={current.posterUrl} alt={current.title} draggable={false} className="w-full h-full object-cover pointer-events-none" />
        ...
        {current && (
          <div className="flex items-center justify-center gap-6 mt-8">
            <button onClick={() => commit("left")} disabled={!!leaving} className="w-14 h-14 rounded-full border border-rose-500/40 text-rose-400 text-2xl hover:bg-rose-500/10 transition-colors disabled:opacity-40" title="Dismiss (←)">✕</button>
            <button onClick={() => commit("right")} disabled={!!leaving} className="w-14 h-14 rounded-full border border-emerald-500/40 text-emerald-400 text-2xl hover:bg-emerald-500/10 transition-colors disabled:opacity-40" title="Wishlist (→)">♥</button>
          </div>
        )}
      </main>
    </div>
