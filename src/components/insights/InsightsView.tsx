...
function ItemCardRow({ items, label, onClose }: { items: InsightItem[]; label: string; onClose: () => void }) {
  ...
  return (
    <div className="mt-3 rounded-xl border border-border bg-neutral-900/60 p-3">
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs text-text-secondary">{label}</span>
        <button onClick={onClose} className="text-xs text-text-secondary hover:text-text-primary">Clear ✕</button>
      </div>
...
function DivergenceRow({ item }: { item: DivergenceItem }) {
  ...
  return (
    <button
      ...
      className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-surface-elevated text-left transition-colors"
    >
      <span className="flex-1 text-sm truncate text-text-primary">{item.title}</span>
      <span className="text-xs tabular-nums text-text-secondary">you {you.toFixed(1)} · crowd {crowd.toFixed(1)}</span>
      <span className={`text-xs font-semibold tabular-nums w-12 text-right ${positive ? "text-success" : "text-danger"}`}>
          ...
          return (
            ...
            >
              ...
              <div
                ...
                style={{
                  height: `${(d.avg / 10) * 100}%`,
                  background: d.avg >= baseline ? "var(--color-success)" : "var(--color-neutral-600)",
              ...
              <span className="text-[9px] leading-none text-text-secondary">×{d.count}</span>
            </div>
          );
        })}
      </div>
      <p className="text-[11px] text-text-secondary mt-2">Green = above your {baseline.toFixed(1)} average.</p>
    </div>
  );
}
...
function MostWatchedColumn({ title, facets, baseline }: { title: string; facets: FacetStat[]; baseline: number }) {
  return (
    <div>
      <h4 className="font-mono text-[11px] font-semibold uppercase tracking-wide text-text-secondary mb-1.5">{title}</h4>
      {facets.length === 0 ? (
        <p className="text-xs text-text-secondary px-1 py-2">Not enough data.</p>
      ) : (
            ...
            {typeOrder.map((t) => (
              <div key={t} className="rounded-xl border border-border bg-surface-elevated p-3">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-medium text-text-primary capitalize">{t}s</span>
                </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            ...
            <div key={col.title} className="rounded-xl border border-border bg-surface-elevated p-3">
