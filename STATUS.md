...
| 🟢 | **Catalog growth: all five phases BUILT. What is left is data.** | **Nobody.** Request cost and memory both stopped scaling with the catalog: `find()` warm **384–610 → 32 ms**, the first request after a rating **397 → 117 ms** (the scoring pass is a chunked background job now), and a warm request's retained heap **109.6 → 39.9 MB** — the facet cache had been holding every parsed provider blob, which is 76% of an entry. All output byte-identical. **Phase 4 steps 1–2 shipped**: the derived projection is stored on disk (§15) and a CONTENT change now PATCHES the pool instead of rebuilding it (§16) — **5 ms for a 10-item ingest against an 87 ms rebuild**, and a patched pool was compared against a rebuilt one on the real catalog (vocab, every IDF weight, 400 items' facets: identical). **Switch it on when you want it**: `BACKFILL_ENABLED=1` starts the paced ingest toward 30–50k titles (~2 pages/30 min, because 30–50k titles is 60–120k provider calls and every row is WAL Litestream ships). Then `CATALOG_BROWSE=1` once `/api/health`'s `catalog.browse.windows` says a type is ready, and browse costs **zero** provider calls. `HOUSEKEEPING_START_MB` reclaims blobs above 1.2 GB. §17: **the provider terms are read.** TMDB caps caching at **six months** — an AGE cap a refresh tier satisfies, and **nothing was enforcing it** (first silent breach ~2026-12-02); `lib/retention.ts` now does. Steam states no limit. ⚠️ **IGDB is the one blocker on the backfill**: the Twitch DSA allows a 24-hour cache or prior written authorization, which its own webhooks product contradicts — **one email to partner@igdb.com is what unblocks phase 4.** ⚠️ **Phase 2's remaining half is blocked on phase 4, not phase 3** — the browse window holds 153 items and 128 of them are rows the provider feed itself wrote (§14). What DID ship: a logged-out search now asks our catalog first, and the provider search is cached (666 → 4 ms). → [docs/catalog-growth.md](docs/catalog-growth.md) |
