...
| ⬜ | **Prod `rr.db` is 331 MB, was 37.7 MB on 2026-08-12** | **Next session.** Probably `facet_page_cache` (persisted L2, added 2026-08-13, stores page payloads and is bounded by entry count, not bytes). One `GET /api/dev/dbsize?deep=1` settles it. Not the WAL, which is a known high-water mark. → [TASKS.md](TASKS.md) |
