...
| 🔵 | **Catalog growth: phases 0–3 done, phase 2's browse half deferred, phase 4 is next** | **Nobody.** Request cost and memory both stopped scaling with the catalog: `find()` warm **384–610 → 32 ms**, the first request after a rating **397 → 117 ms** (the scoring pass is a chunked background job now), and a warm request's retained heap **109.6 → 39.9 MB** — the facet cache had been holding every parsed provider blob, which is 76% of an entry. All output byte-identical. **Phase 4 step 1 shipped**: the derived projection is stored on disk, so the memory cap no longer decides rebuild cost (495 → 171 ms at the phase-4 ratio, output byte-identical). ⚠️ **Step 2 comes before the backfill**: a 50k rebuild is still ~3.4 s until a CONTENT change is incremental rather than a full rebuild. §15 says how, and why the stored raw facets now make it possible. ⚠️ **Phase 2's remaining half is blocked on phase 4, not phase 3** — the browse window holds 153 items and 128 of them are rows the provider feed itself wrote (§14). What DID ship: a logged-out search now asks our catalog first, and the provider search is cached (666 → 4 ms). → [docs/catalog-growth.md](docs/catalog-growth.md) |
