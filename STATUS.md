...
_Last updated: **2026-09-03**. State only, per the doc conventions in AGENTS.md: what is true now, not what happened when. The session-by-session story lives in git log and [the archive](docs/archive/history.md)._
...
| 🔵 | **The taxonomy sweep is built and waiting on decisions** | **You.** `/dev/scoring` → Taxonomy → **Review**, new 2026-09-03. On PROD, **10,639 of 11,222 tag strings sit in Other (95%)**; the rules claim 1,455 of them over 40,839 appearances, which is **91% of everything appearing 20+ times**. 217 cards: 13 tag batches, 67 franchise merges, 137 missing franchise members, each with accept / deny / correct. ⚠️ **Nothing is stored but the NOs** — suggestions are recomputed live, so the numbers on screen are that database's, not a snapshot. ⚠️ **Two categories are PROPOSED, not created**: Mechanics (the genuinely missing axis, ~100 tags) and Perspective & View (weaker). ⚠️ The remaining ~9,000 in Other appear three times or fewer; those are a bulk-select job in the Tags table, not a card each. → [TASKS.md](TASKS.md) item 2 |
