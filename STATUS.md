...
| | Item | Blocked on |
|--|------|--|
| 🟢 | **Google sign-in is LIVE and PROVEN** | **Nobody. Closed 2026-09-01**, by a real sign-in: `/settings` shows "Connected · @Nils Mlynarek", so the token exchange works and the secret is right. Identity-only (`openid profile`, deliberately **not** the email), so no new category of personal data. **Two things to keep**: ⚠️ **do not upload an app logo**, that alone forces a Google verification submission; and `IDENTITY_ONLY_PROVIDERS` in `syncClient.ts` is what stops `staleProviders()` reading "no sync_log row" as "overdue forever" and firing a doomed sync on every `/library` load. → [the archive](docs/archive/history.md), grep `Google sign-in`, and [[identity-only-auth-providers]] |
