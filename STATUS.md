...
_Last updated: 2026-08-22 (**the Fandex Score now picks its top-N facets by the same weighted quantity the score is made of**, and a breakdown row names the category it was scored under; a single-provider sync no longer makes every other provider look freshly synced, which had frozen Steam/RAWG/TMDB since 2026-08-16). Previously 2026-08-21: **public item urls are now `/{type}/{slug}`** and no longer churn on every deploy; the item page gained a cross-media franchise rail; a franchise bundle now busts the pool cache immediately. Previously 2026-08-20: SEO shipped + Search Console verified; the ads gate was reading 80% crawler and is now filtered; www folded onto the apex; restore drill passed; RAWG quota exhausted._
