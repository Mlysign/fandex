...
_Last updated: 2026-08-18 (AN batch shipped + live — the anonymous surface; platform colour-coding removed site-wide)._
...
**Still on you (see [TASKS.md](TASKS.md) "Needs Nils"):** one infrastructure step — `VACUUM_ON_BOOT=1`, redeploy, remove it (see the DB row below) — and **no prod sweeps left** — the Steam cross-link sweep ran 2026-08-17 (131 games linked, cursor drained) and the Wikidata franchise sweep ran 2026-08-14. What is left is action, not judgement. **The affiliate signup is no longer the next step** (direction changed 2026-08-19, see below); what remains is **the TWA decision**, which has a full writeup in TASKS.md. `PRUNE_ON_BOOT=0` is optional now that the guard has held twice in prod. The 340 MB WAL high-water cannot be reclaimed *while Litestream runs* (`busy: 1`, tried twice) — **corrected 2026-08-19:** the `VACUUM_ON_BOOT` step above runs before Litestream attaches, which is the window where it can, and it truncates the WAL as a side effect. The volume is 12% used either way.
...
| 🟡 | **Prod `rr.db` 37.7 → 331 MB — measured and fixed in code; one Railway variable left** | **You:** set `VACUUM_ON_BOOT=1`, redeploy, then remove it. `facet_page_cache` was **24,953 rows / 222.8 MB, 80.2% of the file** (measured, not guessed). No thin-write leak — `media_items` is 2,021 rows / 0.3 MB, below PR17's 2,267. The table is now capped and gzipped and sweeps every 15 min, but deleting rows does not shrink the file. → [TASKS.md](TASKS.md) |
