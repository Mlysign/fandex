...
| 🟡 | **Advanced search's Fandex Score (SM43–45)** | **Fixed and deployed 2026-08-13**, in three parts: the local path's pending flag (SM43), a latency budget on the heal loop (SM44 — 66.3 s → 4.1 s with RAWG down), and the actual cause of the report, the *database-results* half carrying no score fields at all (SM45). **SM46 (2026-08-13) then rebuilt the database half to the spec you gave**: external results are thin-written so they get a uuid, a Fandex Score and a heal path (`tower defense`+`strategy` **0 → 40/40 scored**); multiple tags are ANDed **at the provider**, not intersected afterwards (which returned 0 — two ~40-row samples of huge tags rarely overlap); a missing Fandex Score renders blank rather than a community `/10` standing in for it; and a browse budget took `facet-fetch` from **66.1 s → 6–9 s** with RAWG down. Open: the IGDB cross-link backfill for RAWG-only game rows, and an eyeball on the shimmer/blank states. → [TASKS.md](TASKS.md) |
