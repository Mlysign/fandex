...
| | Item | |
|--|------|--|
| 🔵 | **H2 — Data-model hardening** | **In progress** ([TASKS.md](TASKS.md) H2a/b/c). **H2a ✅ done** — `raw_data` was ~94% of the DB; now projected at write time: **149.6MB → 35.4MB (-76%)**, DB file **160MB → 42MB** after VACUUM, verified lossless against the real catalog. **Next: H2b** (discover persists at enrich → every item gets a uuid → deletes P13's source-id workarounds → ungate `/discover`), then **H2c** (login-with-intent). |
