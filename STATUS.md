...
| | Item | Blocked on |
|--|------|--|
| 🔵 | **Google sign-in is built; the Google side is DONE and IN PRODUCTION** | **You: two env vars in Railway.** (2026-09-01.) Project `fandex` / client "Fandex web" is created, published, and needs no verification review (one domain, no logo, `openid profile` only). ⚠️ **Do not upload an app logo** — that alone forces a verification submission. Until `NEXT_PUBLIC_GOOGLE_CLIENT_ID` + `GOOGLE_CLIENT_SECRET` are set the button does not render and the route 404s, so nothing is live for visitors yet. It is **identity-only**: no library, nothing to sync, and it requests `openid profile` and deliberately **not** the email, so no new category of personal data is stored. ⚠️ Shipping it surfaced a latent trap and the fix is in: `staleProviders()` reads "no sync_log row" as "overdue", so a google identity would have been due FOREVER, firing a doomed `POST /api/sync` and the syncing spinner on every load of `/library` and `/wishlist` (silently, since `providerQueue` is registry-filtered and answers `done`). `IDENTITY_ONLY_PROVIDERS` in `syncClient.ts` is the guard. → [TASKS.md](TASKS.md) "Needs Nils" item 1 |
