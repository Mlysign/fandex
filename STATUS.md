...
| 🟢 | **Facet under-linking is FIXED, and the swept facets are in the sitemap** | **Nobody. 2026-09-02.** Was 876 of 2,691 rendered items linkable across the 56 facets `/` links (33%); now **419/419 on a swept facet**, measured on the anonymous path. `src/lib/facetSnapshot.ts` sweeps a bounded set daily off the request path, the shape the home and calendar snapshots use. ⚠️ **Only the SWEPT facets are advertised** — the original objection (thousands of `force-dynamic` fan-out URLs invite the crawl that grew `facet_page_cache` to 222 MB) still holds for the rest, so **do not widen the sitemap without widening the sweep**. ⚠️ The rows are `browsed = 1`, hence outside `POOL_WHERE`: no scoring impact and they add nothing to the item sitemap. Watch `/api/health` → `facetSweep`. → [docs/seo.md](docs/seo.md) |
| 🟢 | **Discord sign-in is LIVE, and connecting an account that already exists now MERGES** | **Nobody. 2026-09-02**, both proven by real use. Discord is the second identity-only provider (`scope=identify`, no email, no `guilds`, both tokens discarded). Connecting a provider owned by another Fandex account used to say "connected successfully" and do nothing; it now folds the two together, with a **form** to resolve overlapping titles when both accounts hold some. ⚠️ A merge switches which user the session is for and deletes the emptied `users` row, so it mints a new cookie and moves rows through the schema-derived `userScopedTables()` erasure uses. → `src/lib/accountMerge.ts` |
