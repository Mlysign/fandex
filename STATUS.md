...
| | Item | |
|--|------|--|
| 🔵 | **H2 — Data-model hardening** | **In progress** ([TASKS.md](TASKS.md) H2a/b/c). **H2a ✅ done** — `raw_data` was ~94% of the DB; now projected at write time: **149.6MB → 35.4MB (-76%)**, DB file **160MB → 42MB** after VACUUM, verified lossless against the real catalog. **H2b ✅ done + DEPLOYED 2026-07-17** (verified live: anonymous `/api/discover` 80/80 uuids on browse, 38/38 search, 20/20 load-more; item page 200 with no redirect hop). Discover now persists every item it returns (no network; from the list payload), so every item has a uuid: P13's source-id URL machinery is **deleted** (−202/+103, one URL form always uuid) and `/discover` is **ungated for anonymous visitors**. Needed migration 8 (`media_items.browsed`) to keep browsed items out of Best-match/Insights/IDF. **Next: H2c** (visible controls + login-with-intent) — anonymous can now browse, but still gets a sign-in hook instead of the real stars/wishlist buttons. |
