...
_Last updated: 2026-08-27 (evening). **Catalog growth is the live thread**: Nils agreed the shift to serving anon pages from our own DB, and phases 0–3 part-shipped the same evening → [docs/catalog-growth.md](docs/catalog-growth.md). Earlier that day: the 12th smoke test worked through, the filter sheet rebuilt, and the Your platforms / What you track settings._
...
| 🔵 | **Catalog growth: phases 0–3 part-shipped, phase 3 has a remainder** | **Nobody.** Discover reads stored availability, thin rows heal on a timer, an empty provider section falls back to the catalog, and scores are precomputed (`find()` 384–610 ms → 32 ms, output byte-identical). Next: move the score recompute off the request path, then pool memory, then the seeded backfill. → [docs/catalog-growth.md](docs/catalog-growth.md) |
| 🔴 | **BOTH games providers are down on prod** | **Nobody, but it wants eyes.** Measured 2026-08-27: RAWG latched on its quota 401 (known) and **IGDB answered 9 of 9 requests with network errors** (new), so `/api/discover` returns movies and shows only. Discover now falls back to stored games, so the page is not empty, but new games stop arriving. `id.twitch.tv` answers 200, so the token is fine. → [TASKS.md](TASKS.md) "Needs Nils" item 2 |
