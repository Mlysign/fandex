...
| 🟡 | **RAWG monthly API quota is EXHAUSTED** | **Nobody, until it resets.** Measured 2026-08-20: `401 {"error": "The monthly API limit reached"}`. **Not the 2026-08-17 outage** — different failure, different fix. The cross-link sweep cannot run (157 games still lack a RAWG link); games are unaffected because IGDB carries them. ⚠️ **What burns 20k requests/month now has TWO candidates, and the live counter favours the second.** (a) The facet crawler: 4 RAWG calls per cold `/tag/` page × 24,953 `facet_page_cache` rows is ~100k calls, and that same crawl inflated the ads gate. (b) **Something calls RAWG continuously with no human traffic:** `/api/health`'s `providerCalls` read **4 RAWG requests in the first 31 s of a fresh container**, ~465/hour, every one a guaranteed 401. That alone clears 20k in under two days. **Do not pick one by reasoning** — this repo has mis-diagnosed a resource ramp twice that way. The concrete fix either way is the **401/403 latch** (stop calling a provider that has 401'd; `api.letterboxd.com` and `www.omdbapi.com` are doing it too). → [TASKS.md](TASKS.md) |
