...
_Last updated: 2026-09-01. **`/api/*` responses are compressed now, and the catalog backfill turned out to have finished without getting where it was going.** `/api/library` is **9.77 MB → 2.23 MB** (4.39x), live on prod, and Railway's edge passes it through unchanged, which was the one question that needed a deploy to answer. The cause was Next's own: `send-response.js` copies a route handler's headers with `appendHeader`, which stores even a single value as an ARRAY, so the `compression` middleware Next installs on every request rejects every JSON response in the app at its `compressible(Content-Type)` filter. Pages skip that path, which is why they were the half that worked. Also today: **Google sign-in is confirmed working** (Nils completed a real sign-in, so the token exchange is proven and that item closes), the **Fandex Score's tooltip colour** is fixed and the ramp now lives on its design tokens instead of being written twice, and **Railway's region was already in the privacy policy** (Netherlands, `europe-west4`), so that obligation was never actually open. ⚠️ **The backfill finding needs a decision, not a wait**: all three `:future` lanes read `exhausted`, and the future window is a rolling 18 months that DRAINS, so movie went 49 → 35 and show 49 → 26 in four days against a `min` of 200. `CATALOG_BROWSE=1` is unreachable by waiting. Previously (2026-08-31): **The ads gate was reading 62% and the honest number is 4%.** Not a counting bug: `isCrawlerUserAgent` has filtered the beacon since 2026-08-20, but `/dev/analytics` still summed the days before it, and 5,792 of prod's 6,208 pageviews live there (5,769 on the single crawl day 2026-08-20, against 416 real ones since). Every read on the page now starts at the first clean day, the excluded total is shown rather than dropped, and a rejection counter (migration 26) sits beside the pageview count so "is the filter working" is a number instead of an assumption. The user-agent list also grew by ~40 agents that reach a rendered page — and shrank by the five regional engine names that would have deleted real visitors, since YandexSearch, Cốc Cốc, NAVER, Daum and Sogou Explorer are browsers people use. Previously (2026-08-29): **Fandex can now report its own KPIs to the portfolio hub.** `GET /api/telemetry/kpi` is a second, narrow door beside the session-gated `/dev/analytics`: a key in `X-BW-Admin`, compared in constant time, 404 for everybody without `KPI_READ_KEY`, which is set in Railway as of today. The gate was re-verified from outside AFTER a valid key existed, which is the check that actually proves it: no header, a wrong key and an empty header value all still 404. The one judgement call in it is worth knowing: `runsTotal` counts only from 2026-08-21, the first whole day the crawler filter was running, and reports the ~80%-bot era before it as `simRuns`. Previously (2026-08-28): **The Filters sheet works on all three My Stuff tabs now.** Wiring the Progress tab's toolbar exposed that person, studio and franchise pills had matched NOTHING on Library and Wishlist since July: both routes now ship server-computed `facetIds` and the client-side derivation is deleted, so "Rebecca Ferguson" goes from 0 titles to 5. Pricing that payload turned up the bigger thing — **`/api/*` responses are not compressed at all**, so `/api/library` is 8.63 MB where it could be ~2.0 MB (see the table). **Catalog growth is DONE and the backfill is RUNNING on prod.** All five phases shipped over 2026-08-27/28; the plan doc is now a short runbook → [docs/catalog-growth.md](docs/catalog-growth.md), with every measurement and the phase history in [the archive](docs/archive/history.md). Nothing on a request path scales with the catalog any more: `find()` warm 384–610 → 32 ms, a warm request's heap 109.6 → 39.9 MB. **The only thing left is waiting** for the browse windows to fill. Also 2026-08-27: the 12th smoke test worked through, the filter sheet rebuilt, and the Your platforms / What you track settings._
...
| | Item | Blocked on |
|--|------|--|
| 🟢 | **Google sign-in is LIVE and PROVEN on prod** | **Nobody. Closed 2026-09-01**, by Nils completing a real sign-in: `/settings` shows the Google card as "Connected · @Nils Mlynarek". The token exchange was the one unproven link, so the secret is right. The rest of this row is history now. (2026-09-01.) A visitor with no Trakt/Steam/TMDB/RAWG account now has a door. Project `fandex` / client "Fandex web" is published and needs no verification review (one domain, no logo, `openid profile` only, and it does **not** request the email). ⚠️ **Do not upload an app logo** — that alone forces a verification submission. Verified on prod: the route 307s to Google with the right client id, `/settings` renders the Google card (so the `NEXT_PUBLIC_` value reached the client bundle via the Dockerfile `ARG`), and the consent screen loads clean. ⚠️ **The token exchange is the one unproven link** — it is the only step that needs the secret to be CORRECT rather than present. → [TASKS.md](TASKS.md) "Needs Nils" item 1 | It is **identity-only**: no library, nothing to sync, and it requests `openid profile` and deliberately **not** the email, so no new category of personal data is stored. ⚠️ Shipping it surfaced a latent trap and the fix is in: `staleProviders()` reads "no sync_log row" as "overdue", so a google identity would have been due FOREVER, firing a doomed `POST /api/sync` and the syncing spinner on every load of `/library` and `/wishlist` (silently, since `providerQueue` is registry-filtered and answers `done`). `IDENTITY_ONLY_PROVIDERS` in `syncClient.ts` is the guard. → [TASKS.md](TASKS.md) "Needs Nils" item 1 |
| 🟡 | **IGDB runs behind a kill switch while the licence question is open** | **You, when they reply.** The provider terms were read 2026-08-28 from the primary sources (§17). TMDB is fine — its cap is on cache AGE (six months), which a refresh tier satisfies, and **nothing was enforcing it** until today (`lib/retention.ts`; the first silent breach would have been ~2026-12-02). Steam states no limit. **IGDB does not resolve on its own**: the Twitch DSA allows a 24-hour cache or prior written authorization, and we already hold 1,008 igdb links indefinitely — yet IGDB's own API ships webhooks whose only purpose is keeping your copy current. **Nils's call 2026-08-28: keep going, with a switch.** `IGDB_ENABLED=0` stops every IGDB call at once (default ON) and `scripts/purge-igdb.mjs` removes what is stored — proven on a copy: 1,008 links and 908 projections gone, **all 2,770 items and all 2,482 user rows untouched**, 100 items would be left source-less and none of them are acted on. Still worth asking `partner@igdb.com` whether a webhook-refreshed local mirror is covered by the free tier. → [TASKS.md](TASKS.md) "Needs Nils" item 2, [docs/catalog-growth.md](docs/catalog-growth.md) |
| 🟢 | **The portfolio hub's KPI feed is LIVE and keyed** | **Nobody.** `GET /api/telemetry/kpi` answers the shared contract; `KPI_READ_KEY` was set in Railway 2026-08-29 and the URL and var name went back to the Gets session, which writes the PHP proxy. Gate re-verified from outside AFTER the key existed: no header, wrong key and empty header value all 404, so a valid key did not open the door for everybody. ⚠️ `runsTotal` deliberately excludes the pre-2026-08-21 counters (the ~80%-bot era) and reports them as `simRuns`. It used to read smaller than `/dev/analytics` for that reason; as of 2026-08-31 the two AGREE, because the dashboard applies the same exclusion rather than the KPI route widening to match. → [the archive](docs/archive/history.md), "The portfolio KPI route" |
| 🟢 | **SM50 is FULLY REPAIRED on prod, and so is the damage the repair script could not see** | **Nobody. 2026-09-01**, over `railway ssh`. `--apply` ran: 3 repaired, 4 blobs scrubbed, re-run reports **0 offenders**. Two further layers, both found by **reading the rendered page after the repair reported success**: the three movies wore the show's **slug** (`/movie/spongebob-squarepants` served *Being John Malkovich*), and one carried a Wikidata **`item_ip_override`** that put a "More from SpongeBob SquarePants" rail on it. Both fixed by the new `scripts/repair-cross-type-slugs.mjs`; all nine urls re-verified live. ⚠️ **Two things not to "tidy" later**: 43 of the 46 cross-type shared slugs are CORRECT (slugs are unique per TYPE, so `/game/batman` and `/movie/batman` both belong), and 52 of the 53 IP overrides a naive title-match test flags are correct and are the feature working. → [TASKS.md](TASKS.md) item 7 |
| 🟢 | **`/api/*` responses are compressed. DONE and live** | **Nobody. 2026-09-01** (`a4edd5d`). `/api/library` **9.77 MB → 2.23 MB (4.39×)**, and Railway's edge passes it through unchanged: `GET https://fandex.org/api/discover` answers `Content-Encoding: gzip`, `Content-Length: 12901`, `Vary: Accept-Encoding`. Every gzipped body gunzips byte-identical, every one carries a correct `Content-Length`. The cause was Next's, not Railway's, and not `compress: true` being off: `send-response.js` copies a handler's headers with `appendHeader`, which stores a single value as an ARRAY, and `compression`'s filter needs a string. ⚠️ **This devalues the field-trimming idea** below it in TASKS.md: its 4.7 MB was priced on the RAW payload, and those fields gzip too. → [the archive](docs/archive/history.md), grep `gzip /api/` |
| 🟢 | **The Fandex Score's colours agree across all three surfaces now** | **Nobody. 2026-09-01** (`fcfe431`). The tooltip hardcoded gold, so its number was score-independent. It calls `fandexScoreColor()` now, and the ramp moved onto `--color-score-*` so it is defined once rather than twice (the tokens existed from day one and nothing read them, so only the unused copy had a light theme). Contrast measured first: dark 7.93–10.43:1, light 3.76–5.92:1, which clears WCAG's 3:1 large-text bar at the tooltip's 24px but **not** 4.5:1 at body size. Dark theme is otherwise pixel-identical. → [the archive](docs/archive/history.md), grep `ramp was written twice` |
| 🔵 | **Calendar filter bar: 22% of a fixed height budget** (SM53) | **Your call.** Not broken, and deliberately unchanged: it is a visual decision on a page already waiting for your eyes. Three options, cheapest first, in [TASKS.md](TASKS.md) item 9. |
...
| 🟠 | **The backfill FINISHED and did not reach its target. Needs a decision** | **You** (2026-09-01). Catalog growth itself is done and its wins hold: `find()` warm **384–610 → 32 ms**, first request after a rating **397 → 117 ms**, a warm request's heap **109.6 → 39.9 MB**, a 10-item ingest **87 ms rebuild → 5 ms patch**, all output byte-identical. ⚠️ **What is NOT true any more is "just wait, then flip one env var".** All three `:future` lanes read `exhausted: true`, and the future window is a rolling **18 months that DRAINS** as release dates pass, so movie went **49 → 35** and show **49 → 26** in the four days since 2026-08-28, against a `min` of 200. Game is 74 (from 29) and also retired. Catalog is 4,476 items. **`CATALOG_BROWSE=1` is unreachable by waiting, for every type.** Two things fell out of the same reading: **`game:past` is retired at page 1 with 0 added**, which is the exact incident `EMPTY_STRIKES = 3` was written to prevent (RAWG is quota-latched, `fetchGamePageAllSources` swallows it and answers `[]`, and three empty pages from a DOWN provider retire a lane as permanently as three from a finished one), and **`resetBackfill()` is wired to nothing** — no route, no script, so a retired lane cannot be revived on prod. → [TASKS.md](TASKS.md) "Needs Nils" item 1 |
| 🟡 | **IGDB RECOVERED; RAWG is still quota-latched** | **Nobody.** Re-measured 2026-08-28 on prod: `api.igdb.com` 9 requests, 8 ok, `lastStatus 200`, circuit closed — so the 2026-08-27 "both games providers down" reading was a transient, not a standing outage. RAWG is unchanged (5 requests, 5 × 401, 277 blocked), which is the known quota latch and no longer touches the facet paths. Games are arriving again. → [TASKS.md](TASKS.md) "Needs Nils" item 4 |
...
- **The restore drill PASSED 2026-08-23**, `ALL TABLES MATCH` on all eight. ⚠️ **IT IS STILL DUE**: migrations 23–26 have landed since, and 23 rebuilds a table. **It needs the Railway Console** — there is no litestream binary and no bucket credential outside the container, so no Claude session can run it. Commands → [TASKS.md](TASKS.md) item 7.
  - **The two halves that COULD be checked locally were, 2026-09-01, and both passed**, which makes a red drill less likely rather than merely unmeasured. **The schema carries nothing Litestream's older SQLite would reject** (zero aggregate-with-`ORDER BY`, no `string_agg`/`concat()`/`jsonb_*`/`GENERATED`/`STRICT`; all four schema `ORDER BY`s sit in a subquery) — that is the exact August failure, where migration 16 parsed on `better-sqlite3`'s 3.53 and not on the ~3.40 Litestream embeds, killing replication for two days with everything else green. And **the real upgrade path applies clean**: a copy of the July backup went `user_version` **6 → 26**, all 20 migrations via `node scripts/migrate.mjs`, then `integrity_check ok` and 8 populated tables. Green tests never exercise that, since every DB test starts fresh.
