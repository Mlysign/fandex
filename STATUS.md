...
## 🟢 Prod is up and deployed — ⏸️ but the restore drill is DUE again
...
| | Item | Blocked on |
|--|------|--|
| ⏸️ | **Backups: the drill is DUE again** | **You, for one paste.** The 2026-08-20 drill passed, but **migration 19 (`media_items.slug`) landed on 2026-08-21**, and a drill proves the backup you had THAT DAY. Verified 2026-08-22: migration 19 uses no 3.44-only syntax, the replica bucket is growing across deploys, and the deploy log has zero litestream errors — so it is *replicating*. **That is not the same claim as restorable**, which is the entire lesson of 2026-08-17. Claude was blocked from running it (the harness denies shell commands in the Railway Console); **the four lines to paste are in [TASKS.md](TASKS.md)**. ⚠️ Railway volume backups are still Pro-only, so **Litestream remains the only copy**. |
