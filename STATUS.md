...
| **Post-launch (future):** | |
| UI/UX overhaul — mobile-first polish (H1) | ✅ **DONE 2026-07-27**, incl. the follow-up mockup-vs-live rebuild (4 rounds) and the small-tasks batch. Direction 2a "Ticket · Calm", full IA restructure (public Home at `/`, `/dashboard`→308→`/wishlist`, new `/calendar` + `/profile`, adaptive nav, Library⇄Wishlist tab). Design system + per-phase changelog: [docs/ui-overhaul.md](docs/ui-overhaul.md) §10; handoff bundle at [docs/design/fandex-handoff/](docs/design/fandex-handoff/); 8 remaining anatomy judgment calls in [docs/mockup-gap-audit.md](docs/mockup-gap-audit.md). Logged-in-verified (5th smoke sweep, 2026-07-27). **Not deployed** — Railway paused until ~1 Aug, so this whole overhaul is local-only so far. |
