...
All 7 tasks complete, executed 2026-08-02. Verification: 485 tests passing, `npm run lint` 0 errors, `npx tsc --noEmit` clean, `npm run build` clean with `/legal/[locale]/[doc]` confirmed `ƒ (Dynamic)`.

- **T1** — Probed `https://fandex.org/api/health`, still Railway's edge 404 (request_id `3IjCjEYMROGHG6NH9I3ezw`). Recorded that the expected ~2026-08-01 billing-cycle reset did NOT restore service — reframed in TASKS.md as a Railway-dashboard action item for Nils, not a wait-it-out situation.
- **T2** — H4.10 compliance review (`docs/compliance-review.md`): two-click reachability table across all 17 real routes, 5 docs-vs-code cross-checks (all passed, zero corrections needed to any legal doc), statutory checklist vs §5 DDG/GDPR/§25 TDDDG. Found one real, previously-undocumented finding: `AppNav`'s "You" slot doesn't link to `/profile` for anonymous visitors (opens a sign-in dialog instead), and `/profile` itself redirects anon away before `LegalFooter` renders — so no anonymous visitor can reach `/legal/*` through the UI at all, contradicting H4.1's own stated rationale. Logged, not fixed (a nav/UX decision, outside a docs-review pass's authority) — also added to AGENTS.md's load-bearing invariants since it's a genuine trap for a future H4.2 session. Second, smaller finding: the ToS doesn't distinguish RAWG's password-transit connect flow from the other three providers' pure OAuth (disclosure gap, not inaccuracy). Also removed an empty, untracked, shell-brace-expansion-debris directory (`src/app/api/auth/{trakt`) found in passing.
- **T3** — H4.9 monetization legal prep (`docs/monetization-legal.md`): web-researched one-pager scoped to the locked v1 model (donations + affiliate only), every claim sourced with URL + access date. Key finding: standard affiliate tracking sets its cookie on the merchant's domain, not Fandex's, so H4.4's §25 TDDDG exemption should survive H3.4 as long as `buildStoreLink()` stays a plain outbound link (flagged unverified per-program, pending confirmation before shipping). Corrected one fact TASKS.md didn't have: the €100k Kleinunternehmer current-year threshold is now an immediate cutoff, not deferred to the following year.
- **T4** — H3.8's Path B trigger written into TASKS.md in place: ads arm = 10,000 pageviews/mo (Monumetric's minimum), freemium arm = 3,500 sustained WAU (corrected upward from the original "roughly 1k+" napkin estimate once TMDB's $149/mo commercial license is actually netted out). Verified a working WAU-proxy SQL query against the real schema; found `users.last_seen_at` is not a reliable activity signal (only touched on RAWG/Steam login, never on ordinary session-cookie revisits) and named a scoped future fix rather than building it.
- **T5** — Investigated the local 48–57 MB `rr.db-wal` against a scratchpad copy (never touched the real `data/` files). Decisive test: a fresh connection to the exact same WAL bytes reclaimed it to 0 instantly, with no other process running — confirmed dev-process-lifecycle artifact (a long-lived `next dev` connection never gets a clean-close signal), not the 2026-07-22 Litestream-stall mechanism (which doesn't run locally at all and would show `wal_autocheckpoint = 0`, not the observed default 1000). Wrote up in `docs/performance-audit.md` §B. Shipped no code change.
- **T6** — Full A–F smoke sweep (the 9th), not scoped like the last two since this batch shipped little user-facing surface. Zero functional findings. Two red herrings investigated and dismissed (React StrictMode's dev-only triple-fetch on Home; `allowEmpty: true`'s intentional 200-on-malformed-JSON on `/api/discover/find`). One useful measurement pinned: Discover's cold-cache paint took 58–60 seconds locally, confirming the scale of the already-deferred pool-cache perf issue with real numbers.
- **T7** — Full verification suite run and passed. STATUS.md given a new dated headline entry plus updates to the "still open"/roadmap sections reflecting H4's near-closure and the prod-down status change. TASKS.md archived three closed sections (the 7th/8th smoke sweeps, most of the H4 sub-task list, and the tag-admin batch's resolved narrative) into `docs/archive/history.md` and reformatted the new 9th-sweep entry to the file's established dense-paragraph style — brought TASKS.md from 306 lines back under the 200-line guard (193 final). Added one new AGENTS.md load-bearing invariant (T2's anon-reachability finding).

**Systems-level recommendation for a future session:** the catalog pool-signature split (`docs/performance-audit.md` §A) is now the single largest remaining perf item and the best candidate for the next supervised (not unattended) pass — this session's T5 confirms the WAL isn't the problem, sharpening the case that the pool-rebuild cost is the real one.
