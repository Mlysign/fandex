...
1. **Episode tracking is FIXED — one action left: open the app and hit Sync once.** The probe answered it: `/sync/watched/shows` carries **no** episode data (`seasons` absent on all 280 entries, every variant), so the pull now uses `/sync/history/episodes`. Locally the same account went `withEpisodes=0` → `withEpisodes=280, episodes=12318`. Prod has the code but still needs **one sync tap** to populate — that same sync now also bulk-fills the episode catalog (250 shows in one pass, measured), so the Up next rail reaches its 10-entry cap immediately rather than needing ~90 Home loads.
...
- **MB14** ✅ **FIXED 2026-08-16.** Per-episode show tracking works end to end: migration 15, lazy catalog fill, batched `/sync/history` push, `/api/episodes`, `<EpisodeTracker>`, and Home's "Up next" rail. Full write-up → [archive](docs/archive/history.md), grep `MB14 — per-episode show tracking`.

  **Root cause — measured, not inferred.** `/sync/watched/shows` **carries no episode data in any variant.** `/api/dev/trakt-shape` against the live account: `seasons` **absent** (not empty — absent) on all 280 entries with `?extended=full`, plain+paginated, and plain+unpaginated (which also silently returned 100 of 280, so it IS paginated). Trakt documents `seasons` as default-on; it is not. The run-time fallback added on deploy four was grafting from a second empty well, and the unit test asserting it worked **passed** — against a mock shaped the way the docs implied. That is how this survived four deploys with tsc, lint, 750 tests and the build all green.

  **The fix:** `/sync/history/episodes` — bulk, one entry per play, deduped on (season, episode) keeping the latest `watched_at`. `/shows/{id}/progress/watched` is documented as the per-show alternative; ⚠️ it lists every aired episode, so only `completed: true` counts. → [[trakt-episode-endpoints]]

  **Verified on the account that measured zero:** `shows=280 withEpisodes=280 episodes=12318 attached=12318`. The item page reads real per-season progress and the rail renders "Silo S3E6". **Prod needs one Sync tap to populate.**
