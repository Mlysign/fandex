...
- **Provider latency isolation** — `G1` ✅ done 2026-08-02. `G2` (is RAWG's outage theirs?) needs you; `G3` (no browse budget on the adapter-mediated fetchers) open. **10th-sweep findings `SM35`/`SM36`** (games load-more + Home's Popular rail are both RAWG-only) are the product half of the same theme; `SM37` is a tap target.
