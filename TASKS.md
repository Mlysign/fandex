...
| SM39 | 🟠 **OPEN — diagnosed 2026-08-14, see the section below** | data/ux | Fandex Score badge | **The score renders far outside 0–100 *on prod*: min −362.3 · p10 −74 · median 93.4 · p90 301.1 · max 557.4**, 21% below 0 and 47% above 100 (The Godfather **−59**). Repro (logged in): `fetch('/api/library')` → percentiles over `fandexScore`. **This is now believed to be config drift, not the formula** — the same code on the same library with the *calibrated* config scores 28–121. → the SM39 section below. Clamping remains the wrong fix either way: `fandexScore.test.ts:239` pins the unbounded range and `:280` pins that additivity survives a sum past 100, and a display clamp would break `center + Σ(reasons) = headline` (itself a previously-fixed bug). |
...
## SM39 — the Fandex Score range 🟠 diagnosed 2026-08-14, two decisions for you

**Finding 1 — prod is very likely running a stale scoring config, ~9× the calibrated gain.** Local `/api/library`, same code, same user, real DB: **min 28 · p10 52 · median 74 · p90 98 · max 121**, **0** below zero, 170 (8.9%) above 100. Prod's SM39 numbers are reproduced almost exactly by re-running that same local library at **K≈50** — p10 **−74** matches SM39 *exactly*, max 567 vs 557.4, 20.5% vs 21% below zero (`node scripts/probe-score-range.mjs data/rr.db`, row H). Local's persisted config is the calibrated `K 5.4 · C 5 · topN 5/3/3/2`.

**The mechanism is structural, so read this before changing any scoring default.** Migration 9 seeds `scoring_config` with `INSERT OR IGNORE` (`migrations.ts:298`) — i.e. once, with whatever `DEFAULT_SCORING_CONFIG` held the day that database first migrated. `getScoringConfig()` then merges `{...DEFAULT, ...stored}`, so **the stored blob wins**. The only other writer is `saveScoringConfig()` via `PUT /api/dev/scoring`. So every later recalibration (K 10 → 25 → 5.4) moved the *default* and left every existing DB untouched — and `calibrate-fandex.mjs`'s own step 3, "PUT /api/dev/scoring to persist into the LIVE DB", was run locally and never against prod. **Any future default change to `scoring_config` or `tag_category` has the same trap.**

- ⬜ **Confirm it:** open `https://fandex.org/api/dev/scoring` while logged in (read-only, admin-gated) and read `mappingConstantUp`/`Down`. If they aren't 5.4, that alone is SM39.
- ⬜ **Fix it:** `PUT /api/dev/scoring` on prod with the calibrated values. No deploy, no migration.

**Finding 2 — a smaller, real range problem survives the fix, and it's a design call.** Even correctly configured, 8.9% print over 100 and none under 0. Cause: Q19 centers the score on your own mean rating (**66.8**), leaving 33 points of headroom up against 66.8 down — while `mappingConstantUp`/`Down` sit **symmetric** at 5.4/5.4, the one shape that cannot fit an off-center range. Measured options: **B** asymmetric K **3.8/11.8** → 1.1% outside, uses the full range but sends a handful of items slightly negative; **E** top-N **3/2/2/1** at K unchanged → **0.1% outside** (min 40, max 100), at the cost of a thinner breakdown and a 46 → 31-point spread; C/D re-tune `priorStrength` on top of B and land no better. Also relevant: **`FandexScoreBadge` makes no 0–100 claim itself** — it prints a bare integer coloured against `center ± 10`, which reads correctly at any scale; the 0–100 target is only `docs/fandex-score.md` §1. So "relabel rather than re-tune" is a live third option, and it is the naked number alone that looks broken.
