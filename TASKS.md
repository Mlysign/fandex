...
- **PL6. Finish removing platform colour-coding. ⬜ Small, and the site-wide claim is currently not quite true.** The 2026-08-18 decision was "no brand hue in ANY state; identify a provider by its LOGO". Found 2026-08-23 while auditing outbound Steam links for PL5: the Steam trailer link in `src/components/item/LowerSections.tsx` still painted itself `#1b9af7` inline — **fixed** (`bce4611`), and it was the last hardcoded hex in that file. **Still outstanding:** `SOURCE_COLORS` in `src/lib/constants.ts` is read by three live surfaces — `src/components/Badges.tsx:62` (border, text and dot all in the source's hex), `src/components/SubBar.tsx:227`, and `src/app/item/debug/page.tsx` (dev-only, arguably fine). ⚠️ **Not swept without asking**, because they may be deliberate. `TYPE_COLORS` (game/movie/show) is a **different axis and stays**. → [[platform-brand-marks-not-colours]]
