...
- **Home progress module** ✅ **2026-08-16** — Home's three counters (library / wishlist / rated) are gone; a carousel of "the episode you need to watch next" takes that slot, after the highlight panels. ONE filter (the preceding episode is watched) and ONE sort: a watch and a release are dated events on one timeline, an entry sits at its LATEST event, newest first, capped at 10. **A 30-day recency FILTER was the first cut and was wrong** — same two dates, but a filter hides a show instead of ranking it and there is no honest cutoff. Ask that question of any "only show it if it's recent" rule. `src/lib/upNext.ts` + `/api/progress` + `<ProgressRail>`. → [archive](docs/archive/history.md), grep `Home progress module`.
  - **The empty rail explains itself** (added the same day, after it shipped blank and a Trakt sync didn't change that): `upNextStatus()` + a one-line empty state + a Sync Trakt button. It separates *no data* / *sync failed* / *still fetching episode lists* / *caught up*, and the episode reconcile writes its own `trakt-episodes` `sync_log` row so "ran and found nothing" is distinguishable from "never ran". **A module that renders `null` when empty must know why before it ships.**
  - **Its catalog heal is BOUNDED — 3 shows per request, 4 s** — because the Trakt pull writes watch state for shows nobody has opened here, so this can be the first thing that needs a show's episode list. It is a separate route/island from `/api/home` for the same reason. Don't fold it in.

  **Still open on MB14 — needs Nils:** the Trakt round-trip itself is **unverified against a live account**. This container has no `data/rr.db` and no provider keys, so the push/pull path was exercised only through unit tests and a keyless local session (marks land as `source: "local"`). Connect Trakt, tick a season, and confirm it appears on trakt.tv — then confirm a sync pulls a Trakt-side change back.
---
