...
|----|:--:|:--:|------|----------------------|
| SM38 | 🟠 | seo/ux | Home · Discover · facet pages | **The anonymous public surface has ZERO clickable items.** Home renders 30 posters, Discover 80 titles, `/person/christopher-nolan` 30 posters — each with **0** `a[href^="/movie/\|/game/\|/show/"]` and 0 `role="button"` cards. Logged in, the same components render 300 real links on `/library`. Item pages are healthy standalone (200, `index, follow`) and DO link **out** to 9 person + 8 tag pages — so the internal link graph is one-directional: **items → facets, facets ↛ items**. Effects: an anon visitor cannot open a single item from the two main browse pages, and a crawler entering any facet page dead-ends. AGENTS.md records the accepted PR13–15 trade-off as "*fewer* clickable titles"; on Home it is now zero, which is a different bargain than the one signed off. Repro (anon): load `/`, `document.querySelectorAll('a[href^="/movie/"],a[href^="/game/"],a[href^="/show/"]').length` → 0. |
| SM39 | 🟠 | data/ux | Fandex Score badge | **The score renders far outside the 0–100 its badge implies.** Across the 1,773 scored items in the real library: **min −362.3 · p10 −74 · median 93.4 · p90 301.1 · max 557.4**; **373 (21%) below 0 and 836 (47%) above 100 → 68% outside 0–100.** Shown as a bare integer on every card (The Godfather **−59**, The Last of Us **433**, Dead Space 362). This is the known consequence of the raw-sum formula and the `priorStrength`/role-weight re-tune is already time-gated below — this run is the evidence that it is **not cosmetic**: a negative score on The Godfather is user-visible today. Repro (logged in): `fetch('/api/library')` → percentiles over `fandexScore`. |
| SM40 | 🟡 | ui | 404 boundary | Every 404 (`/totally-unknown-path`, garbage item uuid, unknown person, unknown tag) correctly returns **HTTP 404 with the branded body**, but `<title>` is the ROOT title "Fandex — your index of every game, movie & show" instead of a not-found title. SM26 fixed titles across all real routes; the 404 boundary was missed. |
| SM41 | 🔵 | ui | `/profile` | "Support Fandex" renders **twice** — once in the link grid ("Help cover the running costs") and again in the `LegalLinks` footer directly below. |
| SM42 | 🔵 | plan | Plan staleness (B block) | `/api/settings` GET → **405** (not the 401 the plan expects) and `POST /api/discover` → **405** (GET-only now). Not product bugs, but the plan's anon-401 probe for `/api/settings` therefore verifies nothing — checklist item 11 needs rewording. |
