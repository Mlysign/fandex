...
1. **Episode tracking is FIXED — one action left: open the app and hit Sync once.** The probe answered it: `/sync/watched/shows` carries **no** episode data (`seasons` absent on all 280 entries, every variant), so the pull now uses `/sync/history/episodes`. Locally the same account went `withEpisodes=0` → `withEpisodes=280, episodes=12318`. Prod has the code but still needs one sync to populate; Home's season catalog then fills lazily at 2–3 shows per load.
