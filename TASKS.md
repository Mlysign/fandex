...
1. **Android TWA (P15/P16): BUILT and RUNNING on the phone; what is left is the Play upload + the 14-day clock.** ✅ 2026-08-22: package built, `android-package/` (gitignored), Railway serving a real `assetlinks.json`, sideloaded on the Pixel 8 with **no address bar** — the asset-links chain is proven on a real device before the store upload. ⚠️ **Next fingerprint step:** Google re-signs the store build, so after uploading the `.aab` you must APPEND the Play App Signing SHA-256 to `TWA_CERT_FINGERPRINT` (comma-separated); the sideload fingerprint alone will not verify the Play copy. Then the App content forms, then twelve testers. **Step-by-step → [docs/twa-play-store.md](docs/twa-play-store.md).** Fandex as a thin Play Store wrapper of the website (your 2026-06-18 decision). **You already have a Play Console account**, 4 days into Bubblewrap's 12-tester test as of 2026-08-22, so the $25, the identity check and — the part that actually costs something — recruiting twelve testers are all done. ⚠️ **But the 12-testers/14-days rule is PER APP, not per account** (Google's help page says "your app" throughout and production access is applied for per app), so Fandex needs its own closed track, its own opt-in link and its own fourteen days. The same twelve people can serve, they just opt in again. **Two apps can test in parallel and the clock runs unattended, so the ~1 h of packaging work is the only thing standing between now and the countdown starting.** ⚠️ The App content forms (privacy policy, Data safety, content rating, target audience) must be complete before a closed release rolls out, i.e. before the clock can start at all.
...
4. **Franchise rail: should it ask the provider for the titles we do not hold?** The rail (2026-08-21) shows catalog rows only and makes **zero provider calls**, which is why a bundled Spider-Man shows one game: the catalog holds **one** Spider-Man game row, not because the franchise is wrong. Adding a TMDB `/collection/{id}` + IGDB franchise top-up, cached per franchise key for 7 days, would list every film and game whether or not you own them. Cost: one provider call per franchise on a cold, crawlable page (~226 franchises today), and for a signed-in viewer those titles become real catalog rows. Neither provider is RAWG, so the exhausted quota is not the constraint. **Claude can build it; it needs your yes.**
