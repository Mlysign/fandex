...
- **Calendar: two things want Nils's eyes on a real browser, not a fix** (2026-08-26, `0bc9b7a` `d36952e` `1026550`). The page is now exactly one viewport tall and never scrolls, which meant **desktop day cells went from 128px to ~97px** so a six-week month fits without one; that is a visible change nobody has looked at outside the measurement. And the **rail cards' hover tooltip is unverified**: `(hover: hover)` is false in the browser pane, so it reports as a touch device and neither the changed code nor an untouched control can be exercised there. Both are a glance on a desktop browser, not work. Checks are in [smoketest.md](smoketest.md) 13j/13ja.
