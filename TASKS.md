...
1. **✉️ One email to `partner@igdb.com` is what unblocks phase 4's backfill.** (2026-08-28, → [docs/catalog-growth.md](docs/catalog-growth.md) §17.) The provider terms were read from the primary sources before sizing the backfill. TMDB is fine — its cap is on cache AGE (six months), which a refresh tier satisfies, and `lib/retention.ts` now enforces it. Steam states no limit. **IGDB is the one that does not resolve on its own.**
   ...
   **What to ask:** whether maintaining a local mirror of IGDB game metadata, refreshed by their webhooks, is covered by the free non-commercial tier, or needs the commercial partnership they advertise. ⚠️ The standing "do not contact TMDB or Trakt about commercial terms" rule is about not inviting scrutiny **while monetizing on a free tier**; IGDB is not in that rule and Fandex is not monetizing yet, so the calculus differs — but sending it is still your call. **Until it is answered, do not scale igdb storage.**

2. **Android TWA (P15/P16): ⏸️ PAUSED BY NILS 2026-08-23 until the developer account is a BUSINESS account.** The app package is built and proven (sideloaded on the Pixel 8 with no address bar), and **the Play Console entry exists**: created 2026-08-23, name `Fandex`, package `org.fandex.twa`, App, Free. Nothing else on the Play side is done, deliberately.
...
3. **RAWG's monthly quota is exhausted, so the cross-link sweep cannot run.** Measured 2026-08-20: `api.rawg.io` answers `401 {"error": "The monthly API limit reached"}` in 0.17 s. **Not** the timeouts and Cloudflare 522s of the 2026-08-17 outage.
...
4. **Optional, not urgent: the GOG affiliate signup.** Demoted 2026-08-19 with the rest of the affiliate plan. Worth one email anyway, because GOG's dashboard is a free click meter on a site that deliberately collects no click data of its own. **Do NOT apply to Amazon**: its 180-day / 3-sale clock starts at signup, and the self-referral shortcut is a terms breach that closes the account rather than a loophole.
...
**SM49, SM50, SM51 and SM52 are fixed; SM53 is a decision for Nils and lives in "Needs Nils" item 8
...
- **🔵 Catalog growth: serve anon pages from our own DB** (Nils, 2026-08-27, agreed in principle; **phases 0–3 done**, `4ab0066`…`c2628c8`). Grow the catalog on a schedule instead of asking providers per request, so a cold page costs 0 provider calls instead of 14 and a provider outage stops emptying Discover. **The progress table, every measurement and the build order live in [docs/catalog-growth.md](docs/catalog-growth.md)** — read it rather than this entry.
  ...
  ⚠️ **Phase 2's remaining half is blocked on phase 4, NOT on phase 3** (§14, measured 2026-08-28). Serving anon browse from the DB needs breadth: the future window holds 59 games, 52 movies and 42 shows, 128 of those 153 being rows the provider feed itself wrote. Discover is also robots-Disallowed and client-rendered, so it has no crawler cost to kill, and `_pageCache` already makes the anon fan-out ~5 calls per 15 minutes across all visitors. What shipped instead: a logged-out SEARCH asks our catalog first, and the provider search is cached (666 → 4 ms). ⚠️ Do not try to skip the provider search on a "good enough" local match — a title is a prefix of its own sequels, so an exact match on "Blade Runner" hides *2049*.

  **Phase 4 steps 1–2 are done.** §15: `media_item_projection` stores the derived {facets, merged} on disk, so `facetCache.derived`'s 6,000-entry cap stops deciding what a rebuild costs (495 → 171 ms at the ratio a 50k pool would sit at). ⚠️ Do not raise that cap instead: 50k × 6.4 KB is ~320 MB of heap. §16: a CONTENT change now PATCHES the pool — 5 ms for 10 items against an 87 ms rebuild — which is what makes an ingest batch affordable. Both verified identical to a rebuild on the real catalog.

  **Next, before the backfill:** (a) the 5-minute pool TTL is now the dominant remaining rebuild, ~3.4 s at 50k landing on whichever request arrives — decide between raising it (the patch path is now proven equal to a rebuild) and moving the rebuild off the request path the way §12 moved scoring, **measured at size first**; (b) §6's provider-terms questions (TMDB is non-commercial at the free tier, IGDB's storage clause) are a reading task nobody has done, and a bulk backfill is exactly what they govern.

  **Then: the seeded backfill to 30–50k.** ⚠️ **It has one known blocker, and it is a design decision, not a tuning one** (§13's last paragraph): `buildEntries` peeks `facetCache.derived` for every pool item, that cache holds 6,000 entries, so a 50k pool would evict what it just wrote and re-derive the whole catalog from blobs on every rebuild. Raising the cap costs ~320 MB of heap. The answer is probably to **persist the projection** so a rebuild is a table scan — size that before starting, not during.
