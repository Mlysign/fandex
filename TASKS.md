...
1. **A tag search's results are mostly NOT in the catalog** — measured post-fix on prod: **1 of 69** items has a local row, vs **11 of 19** for a person facet. So SM45 leaves a tag search's results almost entirely unscored (scoring an unstored item means asking `/api/discover/scores` a question it can only answer "no" to). The browse feed solves this with a discover-time **thin write**; facet-fetch does none. Do we thin-write them too? That changes what a tag search *costs*, so it isn't a quiet fix. **This is the one that decides whether Nils's original query shows scores.**
2. **🟠 `/api/discover/facet-fetch` has no browse budget** — **66.1 s on prod** with RAWG down (an earlier, warmer run read 18.8 s; 68.3 s locally). The user watches a pulsing "Pulling more from the databases…" for over a minute. Exactly SM44's shape on a different route: `tmdbJson`/`rawgJson` in `facetDetail.ts` call `httpFetch` with no `budgetMs`, and `tagTitles` fires several RAWG pages under one `Promise.all`. Note they're **shared with `publicFacetDetail.ts`**, so a fix also touches the public SEO facet pages (which is where the 59.8 s `/tag/telepathy` render came from). → [[facet-cache-and-provider-latency-masking]]
