...
- **Platform integrations** — Hardcover / Open Library / AniList are chosen but not started. See [PLATFORMS.md](PLATFORMS.md).
  - **Hardcover 🔵 — auth gate checked 2026-08-03 and it FAILED; needs a scope decision before any code.** PLATFORMS.md said "verify this first", so it was. Findings: **no third-party OAuth exists** (open feature request only — the roadmap's "Released" `developer api` post is the personal-token API, not OAuth); **tokens expire annually on a shared Jan 1 reset**, so every connected user breaks on the same day; and **there is no app-level credential**, which gates the *metadata* role too — serving catalog reads would mean using one personal token as a shared app key, against Hardcover's own "not meant to be shared", the same finding class that parked Backloggd. Buildable only as paste-your-token (RAWG's `auth: "credentials"` is the precedent). Full write-up in the PLATFORMS.md deep dive.
  - **Second, separate cost nobody had sized:** Hardcover needs a **`book` media type**, and `MediaType = "game" | "movie" | "show"` is threaded through **72 files** that match on a type literal (51 import the type). Only **one** `Record<MediaType, …>` exists (`constants.ts:4`), so **TypeScript will not flag the other 71** — adding `"book"` compiles clean while silently doing nothing in most of them. That plus schema/migration + `matcher.ts` write-path work makes this an epic, not a platform row.
