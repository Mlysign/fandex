...
1. **Set `NEXT_PUBLIC_SUPPORT_URL=https://ko-fi.com/nilsmlynarek` on Railway** — the **only** remaining infrastructure action, and the last thing gating the Ko-fi link. Railway → `releaseradar` → Variables → New Variable. `NEXT_PUBLIC_*` is build-time inlined so it needs the redeploy that adding it triggers. (Attempted 2026-08-12 via the browser; the harness blocks writing to a service settings form, so this one genuinely needs your hands. Everything else on this list that could be automated, was.)
...
2. ✅ **Memory ramp is dead** — `cgroupMb.fileMb` **flat at 74–76 MB** across ~7 h of uptime and a deliberate crawl load, then **reclaimed to 57 → 25** as the box idled, against `limitMb` 7,629. A plateau then a reclaim, not a climb toward 2,000; the reclaim is the stronger half, since it shows the memory was reclaimable page cache all along. `anonMb` fell 466→292 as V8 GC'd. First window long enough to show a plateau (08-07's was ~31 min, which is why it stayed PARTIAL).
...
4. ✅ **Litestream verified 2026-08-12 — by an actual RESTORE DRILL, not just a snapshot listing.** The drill had been "recommended" since June and never done; it is the only thing that proves recoverability, so do it this way again. `litestream restore -o /tmp/restore-test.db` → `RESTORE_OK`, then `PRAGMA integrity_check` → **ok**, and the restored copy compared against live:
   - **Every user table exact** — `user_library` 1912, `user_watchlist` 96, `user_item_state` 2337, `users` 1, `user_identities` 4.
   - **Every REAL catalog row exact** — `media_items` where `browsed=0`: **1994 in both**. The only difference was disposable `browsed=1` thin rows (740 restored vs 18 live), because the restore point (15:54:57Z) predates that boot's prune (16:01:26Z).
   - Replication current: `lag` **6m39s**, snapshot 10.9 MB created 2026-08-12T09:25:34Z, bucket 27.4 MB.
   - **⚠️ Correct a wrong expectation this file carried:** it said to expect a generation *newer* than `18d8221abccc198d`. The generation is **still `18d8221abccc198d`** — and that is the BETTER outcome. A new generation would mean the lineage BROKE; the VACUUM was absorbed within the existing one, so replication has been continuous since 2026-07-21. Retention is 24 h, so `start` tracks the last boot, not July.
5. ✅ **Stale WAL sidecars deleted 2026-08-12.** `rr.db.tmp-shm` (32 KB) and `rr.db.tmp-wal` (0 B), both dated Jul 17. Verified orphans first: their parent `rr.db.tmp` does not exist and no process held them open.

**WAL note — `wal-truncate` CANNOT reclaim the high-water while Litestream runs.** Tried twice on 2026-08-12: both returned `busy: 1` with `walMbBefore` 340.8 → `walMbAfter` 340.8. A `wal-probe` (PASSIVE) in between returned `busy: 0`, `pendingFrames` 241 / `pendingMb` 1 — so PASSIVE checkpointing works fine; it is **TRUNCATE specifically** that needs exclusive WAL access, which Litestream's persistent read lock denies. That lock is the thing making the backup trustworthy, so this is working as designed, not a fault — but **the earlier framing of `wal-truncate` as "the safe reclaim" was wrong: it is safe, and it reclaims nothing.** Don't keep retrying it.

The file is 340.8 MB holding only ~9–241 frames — a high-water mark, not data (a SQLite WAL is reused in place and only shrinks on TRUNCATE). **No space pressure:** the volume is 4,614 MB with **4,086 MB free (12% used)**, and Railway bills volumes on *provisioned* size, so this likely costs nothing. Leave it unless the volume actually fills; the real lever would be a restart with Litestream briefly stopped, which is not worth the risk.
