...
- **🔵 Catalog growth: serve anon pages from our own DB** (Nils, 2026-08-27, agreed in principle; **phases 0–3 done**, `4ab0066`…`c2628c8`). Grow the catalog on a schedule instead of asking providers per request, so a cold page costs 0 provider calls instead of 14 and a provider outage stops emptying Discover. **The progress table, every measurement and the build order live in [docs/catalog-growth.md](docs/catalog-growth.md)** — read it rather than this entry.
  ...
  **✅ ALL FIVE PHASES ARE BUILT (2026-08-28).** Phase 4's machinery (§15 the stored projection, §16 the incremental pool patch, §18 the seeded backfill), phase 2's browse switch and phase 5's housekeeping all ship. Request cost and memory stopped scaling with the catalog: `find()` warm **384–610 → 32 ms**, a warm request's heap **109.6 → 39.9 MB**, a 10-item ingest **87 ms rebuild → 5 ms patch**. All output verified byte-identical.

  **What is left is DATA, not code.** Three switches, in this order:
  1. `BACKFILL_ENABLED=1` — paced ingest toward 30–50k titles. ⚠️ **The pacing IS the safety feature**: 30–50k titles is 60–120k provider calls and every row is WAL that Litestream ships. Turning `BACKFILL_PAGES` up is how this becomes PR16 again. Check Railway spend first.
  2. `CATALOG_BROWSE=1` — once `/api/health`'s `catalog.browse.windows` reports a type `ready`. A ready section costs **zero** provider calls.
  3. `HOUSEKEEPING_START_MB` — already defaulted to 1200; it no-ops below that.

  ⚠️ Watch `/api/health`'s `catalog` block: `retention.expired > 0` is a provider TERM being breached, not a threshold. ⚠️ **It has one known blocker, and it is a design decision, not a tuning one** (§13's last paragraph): `buildEntries` peeks `facetCache.derived` for every pool item, that cache holds 6,000 entries, so a 50k pool would evict what it just wrote and re-derive the whole catalog from blobs on every rebuild. Raising the cap costs ~320 MB of heap. The answer is probably to **persist the projection** so a rebuild is a table scan — size that before starting, not during.
