...
**Source:** the `PLATFORMS.md` re-read. **Nils decided every open question on 2026-08-23** and those answers are baked into the tasks below; the "decided against" list at the end is as load-bearing as the tasks. Execution order is PL1 → PL3 → PL2 → PL4, with PL5 whenever.

⚠️ **PL3 was re-decided the same day after two of its stated facts turned out to be wrong.** The first plan was "widen Steam first, then drop RAWG"; measuring the catalog showed Steam already does the widening and games were never single-source. **The lesson is the repo house rule again: measure the claim before building on it.** Both corrections are recorded inside PL3 rather than deleted, so the reversal stays visible.

- **PL1. Remove OMDb entirely. ⬜** CC BY-NC 4.0 at **every** tier, so it can be removed but never paid for, and it contributes **zero** to the Fandex Score (which reads facets, not ratings). Its key has 401'd since before 2026-08-20, so nothing it provides is on the page today anyway. Removing now takes it off the go-live critical path instead of leaving a licence breach one forgotten flip away. **Delete:** `src/lib/sources/omdb.ts` + `omdb.test.ts`, `applyOmdbScores` in `src/lib/detail/enrich.ts`, both call sites (`src/lib/detail/publicDetail.ts:99`, `src/app/api/detail/route.ts:141`), the `OMDB_API_KEY` row in `src/lib/config.ts`, and its entry in the app-scoped-auth list. **What is lost:** RT score, IMDb rating + votes + link, awards, box office, and the metascore fallback. **What survives:** certification. TMDB already appends `release_dates` / `content_ratings` with no extra call (`src/lib/sources/tmdb.ts:17,21`), and OMDb only unioned into the same field. ⚠️ **Verify on a real movie before deleting**, and check `communityRatings` still renders with TMDB/Trakt alone rather than collapsing to an empty row.

- **PL2. Trakt's free caps are unhandled, and that is a live bug. ⬜** The 2026 free tier is **watchlist 250, ratings 10k, 5 lists** (VIP 5,000 / 20k / 100). Nothing in `src/lib/sources/adapters/trakt.ts` inspects a rejected write, so a user past 250 gets a **silent partial push**. Same shape as the prune invariant: a write that did not happen must be visible. ⚠️ **Surface it, do not throw.** The *pull* is what must throw; a push failure that throws would take the pull down with it and hand `syncProvider` an outage. Report it as a per-provider sync warning the user can read. Independent of monetization; fix regardless.

- **PL4. IMDb CSV import. ⬜ The only item here that adds reach while LOWERING provider cost per user.** Free to the user, no OAuth, no ongoing quota, and it carries an id we already resolve. `PLATFORMS.md` §3 has called it "build this first" since 2026-08-20 and nothing is built.
  - **Matching is local-first.** `imdbId` currently lives inside `media_links.raw_data` for the tmdb/trakt rows, not as an indexed column. Cheapest fix: an **`imdb` pseudo-source row** in `media_links` (`source='imdb'`, `source_id='tt…'`). It fits the existing `UNIQUE(source, source_id)` and `idx_links_source` exactly, so the lookup is one indexed query with **no new table and no new index**. Backfill it from the tmdb/trakt rows already held.
  - ⚠️ **A 2,000-title CSV matched naively is 2,000 provider searches**, which lands straight back on the quota problem this whole section exists to fix. Budget the provider fallback **per import, not per row**, and report "n matched locally, m looked up, k unmatched" rather than silently dropping the tail. An import that gave up must say so.
  - ⚠️ **An importer is a WRITE PATH into `media_items`** and inherits the thin-write rules: insert-only, never bypassing `matcher.ts`. **Imported titles are a real library, so they must NOT stamp `browsed = 1`.** That flag is what the boot prune deletes.
  - Parse all three IMDb CSV shapes (list, watchlist, ratings); the ratings export carries the user's own score and date, which is the valuable half.
  - ⚠️ **Letterboxd is IN SCOPE, and the "$35/yr Pro" line that demoted it was WRONG** (corrected 2026-08-23, verified against Letterboxd’s own Pro page and help centre). **Their export is free for every member.** Nils’s reason for wanting it stands: it is the userbase most worth reaching, and the import is the whole switching cost. Build **one importer with two parsers**: the matching and write path are shared, IMDb supplies the stable-id path (`tconst`), Letterboxd the title+year path.
  - **Letterboxd specifics.** Read only `ratings.csv` and `watchlist.csv` (Nils, 2026-08-23: no diary, reviews, lists or likes). Ratings are **0.5–5.0 in half-steps**, so doubling to our 0–10 is exact and lossless; do not round. Title+year is a **good** key here rather than a fallback, because Letterboxd sources all film data from TMDB, so its strings are TMDB’s: measured 1,112 movies, 1,108 TMDB-linked, and exactly **one** `norm_title`+year collision catalog-wide. ⚠️ **The per-import provider budget above still applies but is far less binding for movies:** TMDB has **no monthly cap** (50 req/s), unlike RAWG.
  - ⚠️ **Confirm the exact column headers against a real export before writing the parser.** Every claim here about file names and columns is from secondary sources; Letterboxd’s own docs do not publish the schema, and one export takes two minutes.
  - ⚠️ **The Letterboxd API is closed to us BY POLICY, not merely keyless.** It is request-only and explicitly excludes "recommendation projects" and anything "that recreates current or planned features of our paid subscription tiers". Fandex is both. **This weakens the "leave the dark connector in place, a key may still arrive" call made earlier the same day**, so revisit it as dead weight. Letterboxd’s own API page points at import/export as the answer, so the CSV route is the sanctioned one.

- **PL3. Drop RAWG's metadata role. ⬜ RAWG stays a CONNECTOR.** RAWG is **4 of the 14 provider calls on a cold `/tag/` page** against a 20k/mo free quota whose paid tier is only 2.5× free (`docs/scalability.md` §4.3); IGDB is free and uncapped. ⚠️ **Two beliefs were corrected on 2026-08-23 by measuring the local catalog, and the second one reversed the plan.** (1) **Steam ALREADY contributes company facets** at `src/lib/facets.ts:208-211`, reading `basic_info.developers` / `.publishers`; `PLATFORMS.md` said ❌ and was wrong, so the "widen Steam first" step this task originally carried was already done. (2) **Games were never single-source.** Of **1,035** games: IGDB **907**, Steam **808** (721 carrying the developer payload), RAWG **741**, and the **IGDB ∪ Steam union is 965**. RAWG uniquely reaches **70** (6.8%), all `browsed = 0` real pool rows, **19 of them in a real library or wishlist**.
  - **Why the cost is softer than the 70 suggests:** dropping the *role* stops new RAWG **calls**; it does not delete the stored `media_links` rows, and facets project from that stored `raw_data`. Nothing vanishes from a page. What is given up is **refreshing** those 70 and any future RAWG-only title entering at all.
  - **The work:** remove RAWG from `fetchGamePageAllSources` and `fetchTrendingGames` (`src/lib/discoverFeed.ts:609,628`), from `src/lib/facetDetail.ts`, and from `discoverFeed.ts`'s cold paths. **Keep** `api/auth/rawg`, `src/lib/sources/adapters/rawg.ts`, the RAWG branch in `crossLink`, and `normalizeRawg` (stored rows must still project).
  - ⚠️ **Touching any games surface means checking ALL of them**: initial browse, load-more (a different code path from browse), trending, calendar-popular, profile. That split is exactly how three single-source bugs hid through the 2026-08-02 outage.
  - **Verify by the invariant, not the happy path:** a games pull must survive **IGDB** being down, with Steam carrying it. The happy path passed throughout the 2026-08-02 outage and proved nothing.
  - **Re-measure after:** a cold `/tag/` page should drop from 14 provider calls to 10, and RAWG's monthly counter should fall to connector traffic only. That counter is readable now that the 401/403 latch has stopped the wasted re-asking.

- **PL5. Read Steam's API terms for commercial use. ⬜ Small, and currently a blank in the table.** Steam's terms have never been read for commercial use, and **PL3 makes Steam more load-bearing, not less**: it is the only source of store tags like Deckbuilding and Tower Defense, and once RAWG's metadata role is gone it is one of only two games sources. An hour of reading, before ads, and the answer goes into `PLATFORMS.md`'s cost column.
