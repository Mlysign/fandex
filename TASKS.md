...
- **🔵 Catalog growth: serve anon pages from our own DB** (Nils, 2026-08-27, agreed in principle; **phases 0–3 done**, `4ab0066`…`c2628c8`). Grow the catalog on a schedule instead of asking providers per request, so a cold page costs 0 provider calls instead of 14 and a provider outage stops emptying Discover. **The progress table, every measurement and the build order live in [docs/catalog-growth.md](docs/catalog-growth.md)** — read it rather than this entry.
  ...
  **Phase 4, step 1 is done** (§15): `media_item_projection` stores the derived {facets, merged} on disk, so `facetCache.derived`'s 6,000-entry cap stops deciding what a pool rebuild costs — 495 → 171 ms at the ratio a 50k pool would sit at, output byte-identical. ⚠️ **Step 2 BEFORE the backfill**: at 50k a rebuild is still ~3.4 s, and what clears it is making a CONTENT change incremental (`getCache()` already patches membership; content forces a full rebuild because un-counting a departing item's vocab needs its RAW facets, which the projection table now stores). ⚠️ Do not raise the facetCache cap instead: 50k × 6.4 KB is ~320 MB of heap.
