...
- **MB7** ⬜ **NOT REPRODUCIBLE in the browser pane.** "The bottom nav scrolls away on Insights." Checked at 375×812 with a session: `position: fixed`, `top` unchanged at 965 across a 1500px scroll, stuck to the bottom, and **no ancestor with a `transform`/`filter`/`contain`/`backdrop-filter`** (the usual things that turn `fixed` into containing-block-relative). **It's the INSTALLED PWA** (Nils, 2026-08-14) — which **rules out the obvious theory**: standalone mode has no Chrome URL bar to hide, so the dynamic-toolbar explanation is dead. Worth checking next: `layout.tsx`'s `viewport` export sets only `themeColor` — no `viewportFit: "cover"`, no `interactiveWidget` — so `env(safe-area-inset-bottom)`, which `AppNav`'s mobile bar pads with, resolves to 0 in standalone. Needs a real-device look; the emulator cannot see it.
...
- **MB10** 🔵 Med · **scoped, needs one call from Nils** — **store links as brand logos, not `name →` chips** (`LowerSections.tsx` "Links"). The full name set is fixed and small, from `normalize.ts`: **TMDB · IMDb · Trakt · Steam · RAWG · IGDB · GOG · Epic Games · itch.io · Wikipedia · Reddit · Discord · Official site**. **CSP blocks external assets**, so remote favicons are out — the marks must be inline SVG.
  - **The call: add `simple-icons` as a dependency, or hand-write the paths?** `simple-icons` is the standard source, per-icon importable, and accurate. Hand-writing brand path data risks marks that are subtly wrong, and there is no icon pack in the tree today (`lucide-react` has no brand glyphs). Recommend the dependency. Either way "Official site" has no brand and keeps a generic glyph, and every mark needs an accessible name — a bare logo is not a label.
