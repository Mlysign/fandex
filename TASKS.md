...
- **Home progress module** ✅ **2026-08-16** — Home's three counters (library / wishlist / rated) are gone; a carousel of "the episode you need to watch next" takes that slot, after the highlight panels. Relevance = preceding episode watched, AND (that watch OR this episode's release) inside 30 days — which is also the abandonment test, so there is no abandoned flag. `src/lib/upNext.ts` + `/api/progress` + `<ProgressRail>`. → [archive](docs/archive/history.md), grep `Home progress module`.
