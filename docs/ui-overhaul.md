...
**Discover's advanced-filter sheet is already done** — H1.6d's `FilterPanel` → bottom-sheet-on-mobile work (2026-07-24) covers this in full; nothing further needed. **A5 (typed search groups) is the one item in this whole H1.6e list still open** — real backend work on `discovery.ts`'s search + `/api/discover/facets` (which already searches people/tag vocab by query, currently gated behind `withUser` — Discover itself is public, so this needs either an anon-safe path or a deliberate "richer search for signed-in only" scoping decision) threaded into the Discover page's existing scroll/infinite-load state machine. Deliberately not attempted in the same pass as the styling sweep below — it's a different risk class (new search behavior on the highest-traffic page vs. a token/class pass on already-working pages).
