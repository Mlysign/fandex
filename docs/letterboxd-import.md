...
- **Misses go to TMDB search. LIVE since 2026-09-03; it was specified here from day one and wired to nothing for eleven days.** `matchLocally` was the whole of it, so every title the catalog did not already hold was reported as "could not be found in our catalog" and then dropped on import: Nils's 607-row export lost 56 films, among them *Reservoir Dogs*, *American Beauty* and *The Intouchables*, none of which TMDB has ever been missing. `resolveMissesAtProvider` is the fallback; `lookupQueue` and `PROVIDER_LOOKUP_BUDGET` are what it always meant to use.
  - ⚠️ **The "2,000 rows means 2,000 provider searches" warning does NOT bind here.** That came from RAWG's 20k/month cap. **TMDB has no monthly cap** (50 req/s, IP-based; the old 40-per-10s limit was dropped in 2019), so the tail costs minutes, not quota. Movies are the cheap medium to import. The budget is **per import, not per row** (400), concurrency 6, and `overBudget` reports what was skipped so **"we stopped looking" stays separable from "it does not exist"**.
  - ⚠️ **It runs at ANALYZE, not at apply, and that is three decisions in one.** The preview stops lying, the lookup is paid for once, and the resolved ids ride into `import_staging` so apply can create the rows. It resolves an **id only** and writes nothing, which is what keeps the anonymous endpoint inside PR15's write gate. **Stage `matched.rows`, never `parsed.rows`** — staging the parsed ones throws the whole pass away while the preview still shows the better numbers.
  - ⚠️ **Both preview lists test `mediaItemId || tmdbId`.** A provider-resolved row has no local uuid until apply, so testing the uuid alone lists it under "could not be found" while the count above calls it matched.
