...
import) on Nils's instruction: "I will only read the tables". Everything below is
the prose and the older capability tables it replaced, kept verbatim because the
deep dives cost real research: Hardcover's usage terms, Backloggd's access
method, BoardGameGeek's read-only XML API2, AniList's competing-service clause,
the MB14 per-episode findings and the Steam tag-search investigation.

**Grep this section for `platform deep dives` or a platform name.** The live tables
carry every capability, cost and licence fact; only the reasoning lives here.
...
## P18 streaming links, boot-time prune, OMDB guard, cache-contraction prep — 2026-08-03 — ✅ ALL CLOSED

Plan: `.claude/plans/2026-08-03-p18-streaming-links-boot-prune-omdb-guard.md`. Executed unattended per `/project-continue-work` while prod stayed down (2026-07-22 outage, unrelated) — nothing here needed a live deployment.

### P18 — closed, and TASKS.md's stated blocker was wrong on both counts

`TASKS.md` had P18 deferred as needing "the JustWatch Content Partner API" plus "a `PROJECTION_VERSION` bump + full-catalog re-projection, the same class of heavy op that blew the Railway compute budget once already." Neither held up:

1. **TMDB already returns a per-region JustWatch `link`** in the same `watch/providers` payload the app already fetches — `project.ts` was discarding it at write time as "an ~87-char JustWatch URL per region that nothing reads." No partner API, no revenue-share deal (declined 2026-07-18 anyway) needed for a region-level "go watch this" link.
2. **A local re-projection genuinely could not have delivered this.** Migration 7's re-projection transforms the STORED blob in place — but the old projection had already stripped `link` before storage, so there was nothing left to recover from a re-projection. The only thing that could restore it was a real TMDB refetch.
3. **The lazy, network-backed self-heal path already existed and was already load-bearing** (`ensureTmdbDetail` / `storeRefreshed` in `enrich.ts`, since H2b): a link whose `projection_version` is behind the current constant refetches on its next detail read and heals permanently. Measured on the dev DB before this session: 36% of `media_links` sat below the current version with zero ill effect. So the "heavy op" framing was importing the wrong lesson from the 2026-07-22 outage — that incident was about writing an unbounded pool of THIN rows on every crawler hit, not about the cost of a version bump per se.

**What shipped:** `project.ts` keeps the region's `link` and records which bucket won (`offerType`) — `PROJECTION_VERSION` → 3. Migration 12 pre-stamps every **non-TMDB** row already at v2 straight to v3 with a single `UPDATE ... WHERE source != 'tmdb' AND projection_version = 2` — a plain integer-column update, no JSON touched, no network — so a bare constant bump wouldn't needlessly refetch IGDB/RAWG/Steam/Trakt (RAWG detail alone costs 4 sub-requests) for a version change that only altered TMDB's shape. TMDB rows heal lazily, one detail view at a time, exactly as H2b already relied on. `normalize.ts`/`merge.ts`/`EnrichedItem` carry the region's `link` + `offerType` through to the client; `LowerSections.tsx`'s "Where to watch" rows are now a direct outbound `<a href>` (no redirect, no pixel — same rule as H3.4's affiliate links) with the mockup's second line ("Stream · included" / "Rent" / "Buy") restored.

**A real bug surfaced and was fixed while wiring this up:** the first `normalize.ts` draft read a stored `region.offerType` field — but a genuine TMDB response never sends that field; only `project.ts`'s own projection synthesizes it. Reading it back that way made `normalize(original) !== normalize(projected)` for every title with any streaming availability (1,698 mismatches on the real catalog, caught by the existing H2a losslessness probe). Fixed by deriving `offerType` structurally in `normalize.ts` too — from which bucket (`flatrate`/`free`/`ads`/`rent`/`buy`) is populated, the same way `project.ts` does — rather than reading a field only the projected side ever writes.

**Verified live**, logged in via `/api/dev/login`: opened *Lock, Stock and Two Smoking Barrels* (a TMDB link at v2 with real DE availability) — first load refetched TMDB, healed the row to v3 (`projection_version` confirmed via direct DB read), and rendered a clickable row to `themoviedb.org/movie/.../watch?locale=DE` with "Stream · included". `/api/detail` latency dropped 1263ms → 558ms on reload (no second TMDB call). Opened *Grand Theft Auto V* (RAWG + IGDB links pre-stamped to v3 by migration 12) while RAWG's circuit breaker was genuinely open in this environment (unrelated timeout) — the game page returned in 88ms with zero RAWG-related log lines tied to that request, confirming the pre-stamp actually prevents a refetch rather than merely being untested.

### Boot-time prune of the browsed tail — default ON

PR13–PR16 stopped the browsed-discover-tail growth at the write side (2026-07-22) but nothing ever reclaimed it going forward between manual `/api/dev/prune` runs. `dbPrune.ts` gained `bootPrune()`, called from `instrumentation.ts`'s `register()` — fire-and-forget (its DB work is deferred one tick past the call via `setImmediate`, so `register()` still returns immediately and Next's boot sequence isn't blocked on a synchronous better-sqlite3 delete loop). Reuses the existing `runPrune()` — same batching, wall-clock budget, and user-row-touched alarm the dev endpoint uses — with conservative settings (`batchSize: 1000`, `budgetMs: 5000`, vs. `runPrune`'s own defaults of 5000/20000) so a routine boot is a blip. Skips under `NODE_ENV=test`/`VITEST`, skips (logged) when free space is under a 300 MB floor or undeterminable, and can be disabled with `PRUNE_ON_BOOT=0`. **Never calls `runVacuum()`** — that stays a deliberate, on-demand `/api/dev/prune` action (VACUUM rewrites the whole file and invalidates the Litestream replica generation).

**Recorded objection, and the real-world result.** The planner flagged that this ships an unattended delete path to a prod nobody could currently observe (deployments paused since 2026-07-22). Nils chose ON-by-default anyway. Verifying it against the real local `data/rr.db` (a routine `npm run dev` boot, not a deliberate prune run) deleted 2,009 legitimately-prunable rows — `user_library`/`user_watchlist` row counts were identical before and after, confirming the safety guard held, and `media_items` dropped from 4,540 to 2,531 exactly as expected. This is the feature working as designed, not an incident, but it's a real, unplanned mutation worth recording plainly rather than glossing over: a boot prune WILL fire the first time it runs against any DB carrying the same accumulated tail, dev or prod.

### `omdbConfigured()` — narrower fix than originally scoped

The originating finding described `omdb.ts` as doing "a bare `process.env.OMDB_API_KEY!` with no guard... making doomed calls rather than short-circuiting" — but `omdbGet()` already had `if (!API_KEY) return EMPTY`, so the unset-key case was already guarded at the point of the fetch. What was missing was the CALLER-side short-circuit `igdbConfigured()`/`traktConfigured()` already provide elsewhere, and an invalid-but-PRESENT key (the actual prod situation per `PLATFORMS.md`) still made a real doomed HTTP call each time — which no purely-local check can prevent without a network round-trip. Added `omdbConfigured(): boolean` mirroring the existing pattern exactly (module-level `const API_KEY`, same as `trakt.ts`'s `CLIENT_ID` — the codebase's real convention, not the lazy-read pattern originally assumed), and guarded `applyOmdbScores()` in `enrich.ts` with it so an unconfigured key skips the call entirely rather than relying on the internal check alone.

### Cache-contraction prep — reading only, tables untouched

Migration 3's "expand-then-contract" for `user_library`/`user_watchlist` never contracted; they're still caches rebuilt from `user_item_state` on every write (`matcher.ts`'s `rebuildCaches`). `DbSizeReport` (`dbSize.ts`) now carries `libRowsWithoutState`/`wishRowsWithoutState` in its cheap (non-`?deep=1`) tier — 0/0 on the dev DB, matching what `dbPrune.ts`'s prune predicate already assumes for one of its four safety clauses. Deliberately reading-only: dropping the tables stays gated on PR17 confirming the same 0/0 on prod, exactly as `dbPrune.ts:14-26` already argues.
...
# 2026-08-13 — advanced search's Fandex Score (SM43–SM48)

Grep: `SM44 heal budget`, `SM45 facet-fetch`, `SM46 provider AND`, `SM47 steam tags`, `SM48 cross-link`.
...
| SM46 | ✅ **FIXED** (`bcd553d`) | `facet-fetch` · `facetDetail.ts` · `PosterCard` | **Nils's spec for how search should work, implemented.** Three defects in the database half, all visible in one screenshot of a tag search: (a) candidates never entered the catalog — `ExtTitle` dropped the provider payload, so `persistDiscoverItems` refused them and they had no uuid to hang a score on; they now get the browse feed's thin write, and `tower defense`+`strategy` went **0 → 40/40 scored**; (b) tags were ORed while `find()` ANDed them, now ANDed **at the provider** (see below); (c) `PosterCard` fell back to the community `/10` whenever the Fandex Score was null, so a page of un-scoreable results looked like the feature had reverted — now blank once we've asked and come up empty, with the fallback kept only for viewers who were never getting one (anon per D-E, cold start). Plus `BROWSE_BUDGET_MS` on `tmdbJson`/`rawgJson`/the IGDB tag pull: **66.1 s → 6–9 s** with RAWG still down. |

**⚠️ AND must be pushed INTO the provider query — intersecting afterwards does not work, and the numbers are the argument.** Each tag pull is a ~40-row SAMPLE of a tag carrying thousands of titles, so intersecting two samples is empty almost by construction: `deckbuilding` returned 29, `tower defense` returned 40, the intersection **0**. Asking each provider for the conjunction samples FROM the intersection instead (TMDB comma lists in `with_genres`/`with_keywords`, IGDB `&`-joined clauses via the new `discoverIgdbByTags`, RAWG's params **plus a verify pass** against each payload's own `genres[]`/`tags[]`, because RAWG's comma semantics aren't dependable). Verified live: `strategy`+`indie` → Balatro, Factorio. **A source that cannot express the conjunction contributes nothing rather than a wider set** — a partial AND is indistinguishable from a correct one in the results.

| SM47 | ✅ **FIXED** (`1850061`) | `sources/steam.ts` · tag search | **Steam added as the games tag source, and it's the one that actually has the vocabulary.** `Tower Defense` **4,080** games · `Deckbuilding` **4,515** · **both 277** — where TMDB+RAWG+IGDB together return **zero** for that pair. Nils's original query now returns **40 games, 38 scored** (Overdungeon 90, ORX 90, Cartridge Defense 89) in 9.2 s. Uses `IStoreQueryService/Query/v1` with `tagids_must_match`, whose groups AND natively — the exact shape SM46's provider-side AND wants. **⚠️ `filters.tagids` is silently IGNORED: HTTP 200 + the entire 260,878-record catalog for every query, so three different tag combos came back byte-identical and all looked correct.** A test pins the right key. `sort: 2` is measured — of 0–5 it's the only ordering that surfaces real titles. Terms + the trap are written up in [PLATFORMS.md](PLATFORMS.md). |

**⚠️ Correction, recorded because it was asserted before it was checked:** Steam tags were **already** reaching the Fandex Score before SM47. `normalizeSteam` sets `out.tags = d.resolvedTags` and `mergeLinks`' `TAG_SOURCES` already lists `steam`, so they have always flowed through `extractFacets` — verified end-to-end on an item whose only source for the tag `Action` is its Steam blob. The earlier claim ("`facets.ts` only reads Steam's developers/publishers, so the tags never reach the scorer") came from reading the per-source block of `facets.ts` and not the merge path that supplies tags. **The gap was search alone** — those tags existed on the ~495 linked games and nowhere else.

| SM48 | ✅ **FIXED** (`8cb6dc9`, `d0340d5`) | `sources/crossLink.ts` · adapters · backfill | **Games now cross-link to every catalog on ingest, not just on wishlist adds.** Both game adapters opened their enrich hook with `if (kind !== "wishlist") return`, so nothing anyone *played* was ever cross-linked — 473 of 1,090 games had no Steam link, i.e. no Steam tags. They were also half-copies (RAWG's fetched Steam, Steam's fetched RAWG, neither touched IGDB). One shared `crossLinkGame` now runs for both kinds, safe on every item because an already-linked source is skipped before any network call, bounded by `crossLinkBudget` (30 searches/pass) and unable to throw (a throw inside ingest reads as a failed PULL → prune deletes the library). Backfill run locally: **Steam 617→858, IGDB 102→957**; games missing Steam 473→232 (the rest genuinely aren't on Steam), missing IGDB 988→133. **Local `deckbuilding` search 7 → 21 items; `deckbuilding`+`tower defense` 0 → 1 (Ratropolis).** RAWG's 349 deliberately not run — RAWG is down. |

**⚠️ Two traps this turned up, both fixed:**

1. **A sweep driven off "what's still missing" never terminates.** 232 games are genuinely not on Steam, so they stay missing however often they're searched — `remaining` would sit at 232 forever while every batch re-searched the same misses. `runCrossLinkBatch` resumes from a **stable id cursor** that advances past hits *and* misses. Pinned by a test that drives a full sweep with every provider returning null.
2. **The backfill over-reported, and only arithmetic caught it.** It claimed 249 Steam links while the table grew by **241** (IGDB: 875 vs 855). A `(source, source_id)` pair is unique catalog-wide, so when another row already claims that appid the link stays there and ours gains nothing — `linkSourceToItem` returns the item it *actually* attached to, and that return value was being ignored. Now compared, with a `cross_link_claimed_elsewhere` log. **~28 local games are in that state** (duplicate catalog rows competing for one store page); resolving those duplicates is a separate question.


## Smoke test 2026-08-12 11th run (archived 2026-08-14)

## Smoke test — 2026-08-12 (11th run) ✅ all five triaged and closed
...
| SM39 | ✅ **FIXED 2026-08-14** | data/ux | Fandex Score badge | **The score rendered far outside 0–100 on prod** (min −362.3 · p10 −74 · median 93.4 · p90 301.1 · max 557.4; The Godfather **−59**). Root cause was prod's hand-tuned gain constants, not the formula → the SM39 section below. Clamping was and remains the wrong fix: `fandexScore.test.ts:239` pins the unbounded range, `:280` pins additivity past 100. |
...
## PL: the platform capability sweep, 2026-08-23 (all six shipped)

_Archived from TASKS.md the same session it completed, per the doc convention. Every item is done and live; TASKS.md keeps a one-line pointer. The reasoning that is still LOAD-BEARING was moved into AGENTS.md's invariants (the two-provider scope split) and into `docs/letterboxd-import.md` (the import), so this block is the record rather than the reference._

**Commits:** `21cfa92` (PL1) · `5fdc0c1` (PL3) · `9a08b68` (PL2) · `022f94e` (PL5) · `2f7c56a` + `fbea871` (PL4) · `bce4611` + `f0b84d7` (PL6).

**The three things worth remembering, if you read nothing else:**

1. **PL3 shipped NARROWER than its own title, and a test is why.** Cutting RAWG from the browse feed failed `discoverFeedSources.test.ts`, which pins the two-source games invariant the 2026-08-02 outage bought. Browse measures **0** RAWG calls; the cold facet page measured **4 of 4**. So the browse cut would have traded a real resilience property for nothing, and was reverted the same day.
2. **PL2's stated cause was wrong.** The Trakt adapter always threw on a rejected write. The silence was `/api/watchlist` answering `{ ok: true }` regardless, and then something larger found while fixing it: **nothing on the client rendered `warnings` at all**, so `/api/library` had been reporting refused writes into the void since H2.
3. **Two `PLATFORMS.md` capability cells were WRONG** and both were planned around before being checked: Steam has contributed company facets all along, and the Letterboxd export is free rather than Pro-only. A capability cell is a claim about code.
...
## Two per-user preferences, shipped 2026-08-27 (platforms + media types)
...
# Catalog growth: serving Fandex from its own database (2026-08-27/28) — ARCHIVED, all five phases done

Proposed by Nils 2026-08-27, built out over two days, **complete 2026-08-28**. The
live operational half — the three env switches, what to watch in `/api/health`, and
the open IGDB licence question — stayed behind in
[docs/catalog-growth.md](../catalog-growth.md), which is now a short runbook.
Everything below is the WHY: what was measured, what the measurements reversed, and
the faults each phase turned up.

**Grep this for:** `fandexScoresFor`, `media_item_projection`, `unfoldEntry`,
`retention`, `backfill`, `housekeeping`, `OCTET_LENGTH`, `IGDB_ENABLED`, or any
number you are about to quote.

**The four findings that reversed a belief, in case you only read one paragraph:**

1. **Memory was never the wall; CPU was.** The plan opened by pricing the scoring pool
   and planning to intern its facet strings. Measured: the pool was **13.5 MB of the
   109.6 MB** a warm request retained, and interning every live facet array saved
   **0.4 MB**. The real hog was `facetCache.derived` holding every parsed provider
   blob — 76% of an entry — which its own header comment forbids in as many words.
2. **Phase 2 was blocked on phase 4, not phase 3**, and the build order said otherwise
   for two days. Serving browse from a database holding 153 in-window titles would
   have been a circular feed that looked like it worked.
3. **A cache key can cost more than the thing it guards.** `librarySignature` asked a
   VIEW for a COUNT and a MAX: 69 ms, three times a request, to produce a cache key.
4. **A comment explaining why something is impossible is a specification for what is
   missing.** `getCache()` had carried a paragraph for weeks saying a content change
   could not be patched because un-counting a vocab contribution needs raw pre-alias
   facets. Nothing about it was wrong; it just named an input nobody had kept.
