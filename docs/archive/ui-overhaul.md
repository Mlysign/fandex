...
- **Infra**: any new/changed public SSR surface must be `BoundedCache`'d and must not add write-on-GET paths (see the 2026-07-20 incident, [[prod-incidents]]) — the new public Home is exactly such a surface.
...
- **`TypeFilter` circles / `SortMenu` dropdown — ✅ done 2026-07-27 (mockup-vs-live gap pass).** Nils's call: switch to the literal mockup anatomy after all. New `ui/TypeFilter.tsx` (03-components.md §6a — circular 40px icon chips, `Layers`/`Gamepad2`/`Clapperboard`/`Tv`, active = accent fill for "All" or the media color for a single type, `text-on-accent`/`text-on-media` icon color) replaces the old text-pill type chips in `SubBar.tsx` and (for consistency) the calendar page's own copy of the same control (`CalendarPageClient.tsx`). `SubBar`'s sort row and `PublicFacetView`'s (person/tag/studio) sort-pill row both became a single `SortMenu`-style trigger (text + `ChevronDown`, mono `text-secondary`) opening the existing `Menu` popover (§6b) — `PublicFacetView` wasn't explicitly named in the ask but carries the identical Q14 pattern (documented in H1.6e's facet-pages section as deliberately matching SubBar's), so it was included for consistency rather than leaving one surface half-migrated. Browser-verified live (both anon and Nils's real logged-in session): Discover, Library, Wishlist, Calendar, `/tag/drama`, `/person/christopher-nolan` — type toggling, sort re-ordering, and the Fandex Score overlay option (facet pages, signed-in only) all confirmed working, zero console errors. 329 tests, typecheck, lint (0 errors) clean throughout.
...
**Discover's advanced-filter sheet is already done** — H1.6d's `FilterPanel` → bottom-sheet-on-mobile work (2026-07-24) covers this in full; nothing further needed.

**A5 (typed search groups) ✅ done 2026-07-26 (Sonnet).** Discover's search now shows three named groups — **People**, **Tags**, **Titles** (in that order) — instead of Titles only, whenever a real text query (≥2 chars) matches the catalog vocab.
- **Reused existing, already-shipped machinery rather than building new backend:** `/api/discover/facets?q=` is the exact same catalog-vocab search `FacetAutocomplete` already calls for the must-include/exclude pills — no new endpoint, no new query logic. A new `FacetMatchGroup` component renders up to 8 matches per kind as `FacetLink` pills (the same component/href-building `FactsSection`/`FandexScoreSection`/facet pages already use), so clicking one navigates straight to the person's/tag's facet page.
- **Scoping decision, made deliberately rather than left ambiguous:** `/api/discover/facets` is `withUser`-gated. Rather than loosen an existing auth-gated route (a change with its own review surface — losing the route's per-user rate limit, needing a new anon-safe limiter), the People/Tags groups are **signed-in only** — anon gets exactly today's Titles-only search, zero behavior change, verified live. This mirrors the established pattern for every other richer-when-signed-in feature (Fandex Score, personalized rails). The underlying vocab data itself is public catalog metadata (same cache `getTagVocab()`/`getCompanyVocab()` already expose to anon on `/tag`/`/studio` pages) — the gate is about the endpoint's shape, not data sensitivity, and could be revisited later if there's appetite to build a proper anon-safe rate limiter for it.
- Company/studio vocab matches are deliberately not shown as a 4th group — the plan's own wording names exactly three groups ("Titles / People / Tags"); showing studios would be scope creep beyond what was asked.
- Also fixed two stray `text-white` instances in the same file (the "No results" search highlight) to `text-text-primary` while already there.
- **Verification:** typecheck clean, `eslint` 0 errors (pre-existing `any`/hook-dependency warnings only, all present before this change). `npm test` 312 passing. Discover is public, so this got a full live check: searching "action" as an anon visitor returns the same Title results as before (verified via network tab — only `/api/discover?q=` fires, `/api/discover/facets` is never called for anon), zero console/server errors. **Not checked:** the signed-in People/Tags rendering itself (needs a real session to trigger the authed fetch) — verified by code/type review instead (the response shape from `/api/discover/facets` matches `VocabMatch` exactly, and `FacetLink`'s href-building reuses the same key-normalization functions the vocab was built with).
...
**Insights ✅ done 2026-07-26 (Sonnet).** No "hours" stat to remove (§8's decision) — grepped `src/components/insights` for `hours`/`Hours` and found zero matches; that decision was already satisfied, nothing was ever built. Retokened all 6 files (`InsightsView`/`OverviewCards`/`Histogram`/`StatBar`/`FacetSection`/the `/insights` page wrapper — `TagCategoryHoverPanel` inside `FacetSection` too), the last logged-in page still on pre-token styling.
- Same contrast fix as item detail/facet pages: `text-neutral-500`/`600`/`700` on body/label text throughout → `text-text-secondary`; panel containers `border-neutral-800 bg-neutral-900` → `border-border bg-surface-elevated`.
- **Semantic-color fixes, not just shade swaps:** the you-vs-crowd divergence deltas and section headers ("You rate higher"/"You rate lower") were stock-Tailwind `emerald-400`/`rose-400` → now `text-success`/`text-danger`; the decade-chart bars' above/below-baseline color and the admin "Saved ✓" confirmation were the same off-brand default greens → `var(--color-success)`.
- **Replaced the page wrapper's bespoke loading/error/empty markup with the shared primitives** (`Spinner`/`ErrorState`/`EmptyState`) instead of inline centered-text divs — the error state gets a real retry button for the first time (previously a plain underlined "Try again" text link), matching every other H1.6e page's error-state shape. `<h1>` gets `font-serif`/`text-serif-xl`.
- **Verification:** typecheck clean, `eslint` 0 warnings, `npm test` 312 passing. Insights is logged-in-only (real per-user ratings), so — same constraint as Calendar/Profile — only the anon auth-gate redirect was browser-verified live (zero console/server errors); the populated charts/facet panels/admin hover-editor were code-reviewed against the same token vocabulary already browser-verified on Home/item-detail/facet pages, not live-rendered with real data.

**Settings ✅ done 2026-07-26 (Sonnet, styling-only pass).** The last page still on pre-token styling — connected-accounts cards, region, account info, Your data, and both inline modals (RAWG connect, delete-account confirmation). Same contrast/off-brand fixes as every other page this pass: `text-neutral-400/500/600` → `text-text-secondary`; panel containers → `bg-surface-elevated border-border`; the notice banner and "Connected" pill's stock-Tailwind `green-900/green-400`/`red-900/red-300` → `success`/`danger` tokens; `<h1>`/section `<h2>`s get `font-serif`. Form inputs inside both modals moved to `bg-surface-inset` (the token file's own "wells inside elevated" role) instead of `bg-neutral-800`, which sat one step LIGHTER than its own modal background — the wrong direction for an inset field. **Provider brand colors (Trakt red `#ed1c24`, Steam blue `#1b9af7`, RAWG green) were deliberately left as literal hex** — §9 already documents these as kept brand marks, not theme.
- **Scope discipline, given this file touches account deletion (H4.6, the one irreversible action in the app) and disconnect/export flows:** this was a classes-only pass — zero changes to any handler, fetch call, or state logic, verified by re-reading the diff before committing. The two bespoke modals (RAWG connect, delete-confirmation) were **not** migrated onto the `Sheet` primitive (which H1.6b built partly to replace exactly this kind of bespoke scrim markup, and which `ConfirmDialog` already uses) — that would add real interaction-behavior risk (focus trap, escape handling) to the account-deletion flow for a styling task. Flagged as a real, worthwhile follow-up (neither modal currently traps focus), not silently dropped.
- **Verification:** typecheck clean, `eslint` 0 errors (4 pre-existing warnings — 3 `any`, 1 unrelated pre-existing unused `logout` function — none introduced by this change). `npm test` 312 passing. Settings is logged-in-only; same constraint as Insights/Calendar/Profile — only the anon auth-gate redirect was browser-verified live (zero console/server errors). The populated connected-accounts list, both modals, and the account-deletion flow itself were **not** exercised live (need a real signed-in session, and the deletion flow specifically should never be triggered outside a deliberate, user-directed test per AGENTS.md).

**Library/Wishlist ✅ done 2026-07-26 (Sonnet) — the D-A tab, not a state merge.** Both pages' own code comments already spelled out the deferred scope exactly: "Per D-A this and /library are one 'my stuff' surface; the Library ⇄ Wishlist tab that unifies them is H1.6e — for now it keeps its own route + content." Built that literally: a new `LibraryWishlistTabs` component (two-segment pill, active state via `border-accent`/`bg-accent-subtle` — the same convention `SubBar`'s sort pills and the facet pages' sort pills now share), rendered above each page's `SubBar`. **Deliberately not** a merge of the two pages' state/filters/data-fetch into one component: each page keeps its own route, its own `usePersistedState` keys (already namespaced `rr_library_*`/`rr_wishlist_*`), its own fetch (`/api/library` vs `/api/calendar`) — the nav already treats them as one destination (H1.6c, both light the "Library" nav icon); this adds the missing way to *switch* between them without leaving the page, nothing more.
- While in both files: fixed the same legacy Sync-button styling (`bg-neutral-800`/`border-neutral-700` → tokens) and a stray `text-white` search-highlight span, plus retokened Wishlist's first-run `OnboardingState` (raw `rgba(74,222,128,…)`/`#4ade80` success-step colors → `var(--color-success)`/`var(--color-success-subtle)` tokens, `<h1>` gets `font-serif`).
- **Verification:** typecheck clean, `eslint` 0 errors (pre-existing warnings only, none introduced). `npm test` 312 passing. Both pages are logged-in-only; browser-verified the anon auth-gate redirect on both `/library` and `/wishlist` (zero console/server errors). **Not checked:** the tab switcher's actual click-through behavior and both pages' populated content need a real signed-in session.

### H1.6f — Cross-cutting QA ✅ DONE 2026-07-27 (Opus, main loop)

Every item in the list below was executed. **Three real bugs surfaced that no test suite would have caught** — all three were invisible-to-typecheck, browser-only failures:

1. **Contrast: several tones were unreadable, not merely dim.** The audit (§9.2 flagged it as an audit, not a find-and-replace) found `GroupedView`'s past-month dividers and weekday names at **1.27:1** and its past day-headers at **1.57:1** — effectively invisible. Poster/cast placeholders sat at **1.48:1**, under even the relaxed 3:1 bar that applies to icons and 24px-bold text. Past states now recede via `neutral-400` (5.03:1, the token file's own documented AA floor) plus a lighter divider rule, rather than by dropping under the floor; "today"/current-month move from raw white to the accent, matching `CalendarView`'s H1.6d treatment. **The 404 page turned out never to have been retokened at all** — it sits outside the IA route list, so H1.6e's page-by-page sweep never reached it. Deliberately kept: `Avatar`'s and `CalendarView`'s icons are non-text and pass the 3:1 bar at 3.23–3.42:1. `dev/scoring` + `item/debug` are admin/debug tooling and were left out of scope.
2. **Reduced motion had a hole the global CSS rule cannot close.** `globals.css` sets `scroll-behavior: auto !important` under `prefers-reduced-motion`, but an explicit `behavior: "smooth"` passed to `scrollIntoView()`/`scrollBy()` **overrides the CSS property** — so all four JS-driven smooth scrolls (the month scrubber ×3, the `Rail` chevrons) silently escaped it. New `scrollBehavior()` helper, read at call time.
3. **SM8 — the Back button was trapped on every gated page.** Found by the smoke sweep, not by inspection: auth gates redirected with `router.push("/")`, leaving the gated route in history, so an anon visitor who tapped a gated nav item and pressed Back re-entered it, got redirected again, and could never return to where they came from. 9 sites (7 gates + both logout handlers) → `router.replace("/")`. Verified by re-running the repro: history delta for one nav click went 2 → **1**, and Back correctly returns to `/discover`.

**Touch targets (Nils's call: pad hit areas, keep the controls' visible size).** New `.tap-44` utility — a transparent `::after` claiming `max(100%, 44px)`. Measured before: sort pills **20px** tall, chips 30px, "Filters" 27px, and the "Clear search" button a **16×16px** target on a high-traffic control. Two things the naive version got wrong, both caught by measuring rather than assuming: the view toggle's three buttons share a ~102px group, so claiming 44px of *width* each made them overlap by 10px and steal each other's taps — worse than the small target, hence the height-only `.tap-44-y` variant (34×44, an honest partial); and the chips and sort pills live in the **same** flex-wrap row, so on mobile they wrap into vertical neighbours 8px apart and their expanded regions overlapped by 11px — fixed with `gap-y-5` on that row, not the outer `space-y` I first reached for. Final measurement at 375 / 500 / 1280px: 11 controls, all ≥44px tall, 9 fully 44×44, **zero overlapping pairs at any width**. **Still open:** `ActionCells` is `h-8` (32px) in card layout / 36px in row layout — only measurable logged-in (anon cards render no action bar), noted in `smoketest.md`.

**Q3 (Library/Wishlist scroll) + the sort default.** `GroupedView`'s auto-scroll targets "today or the next release date" — right for **Wishlist** (forward-looking; "what's next" is the page's purpose, so it's kept) and wrong for **Library**, an archive where that's the far unreleased edge, which is why it dumped you into the TBA pile. Library no longer auto-scrolls. Separately, Nils chose to add a **"Recently added"** sort and default Library to it: deliberately *not* in the shared `SORTS` set (on Discover's live feed every item would tie at null — a visibly dead option) but in a new `LIBRARY_SORTS` list, and deliberately absent from `zSortKey`/`discovery.ts` so the catalog `find()` endpoint can never be asked for a sort it has no column for. `/api/library` now returns `addedAt`; the `user_library.added_at` column has existed since the original schema and was simply never selected. Closed a latent hole while there: widening the shared `SortKey` union left `sortDiscover`'s switch non-exhaustive (TypeScript does not flag this), so a stale persisted value would have returned the list unsorted — it now degrades to release date explicitly.

**Also done:** deleted the orphaned `SearchModal.tsx` (dead since H1.1's "Search nav slot goes to `/discover`") and the stray empty `src/app/api/{calendar,detail,watchlist,sync,search}/` brace-expansion directory. **Verified H1.1's own flagged risk does not materialise:** all four Discover sorts stay populated for anon (40 cards, no empty state) and Popularity genuinely re-orders. Focus rings (`--shadow-focus`, `:focus-visible` only) and the reduced-motion global were already correct from H1.6a. Full gate: typecheck clean, `eslint` **0 errors**, **314 tests** (+2 new), and `next build` compiles — the first production build run this whole stretch.

**Process note worth keeping:** never run `npx next build` while `next dev` is running. It overwrites `.next` and corrupts the dev server into serving 404 HTML, which surfaces as a convincing product bug (`/api/auth/me` 404 → `/calendar` rendering its error state). That cost real time this session and is now recorded in `smoketest.md`.
...
## 11. Post-H1 mockup-vs-live gap pass — 2026-07-27

With H1 fully shipped, went back through `05-DELTA.md` and this doc's own "still open" / "deliberately deferred" notes to close the remaining decision points rather than let them go stale. Nils's calls:
- **Calendar Agenda-default on mobile** — kept Month everywhere (no change; the H1.6d hydration-risk rationale stands).
- **`TypeFilter`/`SortMenu` mockup shapes** — switched to the literal mockup anatomy (§10 H1.6d, above) — the one item where the earlier Q14-precedent call got revisited.
- **Insights "hours" stat** — kept omitted (no change).
- **Extras actually fixed this pass:**
  - **`Sheet.tsx`'s focus-steal bug** (flagged in §10 H1.6e's Settings note) — the mount effect now checks `panel.contains(document.activeElement)` before calling `panel.focus()`, so a child's `autoFocus` (the delete-account confirm's "type DELETE" input) keeps focus instead of losing it back to the panel. Verified live against the real delete-account Sheet (opened, confirmed `document.activeElement` is the input, canceled without deleting) — Nils's own RAWG-linked account, so the dialog was opened and canceled, never submitted.
